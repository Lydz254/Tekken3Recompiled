...
LOADER_HEADER = ROOT / "runtime" / "include" / "overlay_loader.h"


def function_body(source: str, name: str, start_at: int = 0) -> str:
    match = re.search(
        rf"\b(?:static\s+)?(?:int|void|uint64_t|DWORD\s+WINAPI)\s+"
        rf"{re.escape(name)}\s*"
        rf"\([^;]*?\)\s*\{{",
        source[start_at:],
        re.S,
...
def main() -> int:
    ...
    require(r"THREAD_PRIORITY_BELOW_NORMAL", warm_queue,
            "speculative image mapping must stay below the emulation thread")
    require(r"IMAGE_WARM_SMALL_CACHE_MAX_DLLS\s+64", source,
            "adaptive whole-cache warming must retain its 64-DLL bound")
    require(r"IMAGE_WARM_SMALL_CACHE_MAX_BYTES\s+"
            r"\(64ull\s*\*\s*1024ull\s*\*\s*1024ull\)", source,
            "adaptive whole-cache warming must retain its 64-MiB bound")
    warm_seed = function_body(source, "overlay_image_warm_seed_boot_text")
    require(r"compact\s*&&\s*count\s*>\s*0", warm_seed,
            "only a proven compact inventory may take whole-cache warming")
    require(r"region_start\s*!=\s*DIRTY_RAM_KERNEL_WINDOW_END", warm_seed,
            "large inventories must retain scoped boot-text warming")
    require(r"func_count\s*>\s*8", warm_seed,
            "large-inventory fallback must retain the small-bundle bound")
    warm_worker = function_body(source, "overlay_image_warm_main")
    require(r"WaitForSingleObject\(s_image_warm_cancel_event", warm_worker,
            "the image warmer must use a cancellable startup wait")
    warm_shutdown = function_body(source, "overlay_image_warm_shutdown")
    signal_pos = warm_shutdown.find("SetEvent")
    join_pos = warm_shutdown.find("WaitForSingleObject")
    if signal_pos < 0 or join_pos < 0 or signal_pos > join_pos:
        raise AssertionError(
            "image-warmer shutdown must signal cancellation before joining")
    require(r"WaitForSingleObject\(s_image_warm_thread,\s*INFINITE\)",
            warm_shutdown,
            "shutdown must join the image warmer before releasing its state")
    require(r"void\s+overlay_loader_shutdown\s*\(void\)\s*;", loader_header,
            "the runtime must expose image-warmer shutdown")
    if len(re.findall(r"\boverlay_loader_shutdown\s*\(\s*\)\s*;",
                      main_source)) < 2:
        raise AssertionError(
            "process shutdown and lobby teardown must both join the image warmer")
