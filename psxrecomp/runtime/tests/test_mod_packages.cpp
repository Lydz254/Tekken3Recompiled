...
int main() {
    ...
    const std::vector<uint8_t> strided_payload = {1, 2, 3, 4, 5, 6};
    const std::vector<uint8_t> strided_gap = {7, 8};
    const std::vector<uint8_t> strided_collision = {9};
    write_bytes(root / "packages/strided.mod/1.0.0/assets/stream.bin",
                strided_payload);
    write_bytes(root / "packages/strided.mod/1.0.0/assets/gap.bin",
                strided_gap);
    write_bytes(root / "packages/strided.mod/1.0.0/assets/collision.bin",
                strided_collision);
    write_text(root / "packages/strided.mod/1.0.0/manifest.toml",
        "format_version = 6\n"
        "id = \"strided.mod\"\n"
        "version = \"1.0.0\"\n"
        "name = \"Strided Mod\"\n"
        ...
        "target = \"disc_raw\"\n"
        "offset = 200\n"
        "file = \"assets/stream.bin\"\n"
        "sha256 = \"" + sha256_hex(strided_payload) + "\"\n"
        "chunk_size = 2\n"
        "stride = 4\n"
        "chunk_count = 3\n"
        "[[overlay]]\n"
        ...
        "offset = 202\n"
        "file = \"assets/gap.bin\"\n"
        "sha256 = \"" + sha256_hex(strided_gap) + "\"\n"
        "[[overlay]]\n"
        "feature = \"collision\"\n"
        "target = \"disc_raw\"\n"
        "offset = 205\n"
        "file = \"assets/collision.bin\"\n"
        "sha256 = \"" + sha256_hex(strided_collision) + "\"\n");
    check(feature_reload.scan(&error), error.c_str());
    ...
    const ModPackage* strided_package =
        feature_reload.selected_package("strided.mod");
    check(strided_package && strided_package->overlays.size() == 3 &&
              strided_package->overlays[0].chunk_size == 2 &&
              strided_package->overlays[0].stride == 4 &&
              strided_package->overlays[0].chunk_count == 3,
          "format-6 strided overlay layout must survive manifest parsing");
    check(feature_reload.set_feature_enabled(
              "strided.mod", "stream", true, &error), error.c_str());
    check(feature_reload.set_feature_enabled(
              "strided.mod", "gap", true, &error), error.c_str());
    ModResolution strided_gap_plan =
        feature_reload.resolve("SLUS-TEST", {}, overlay_disc_hash);
    check(strided_gap_plan.ok,
          "bytes in a strided overlay gap must remain independently claimable");
    check(feature_reload.set_feature_enabled(
              "strided.mod", "collision", true, &error), error.c_str());
    ModResolution strided_collision_plan =
        feature_reload.resolve("SLUS-TEST", {}, overlay_disc_hash);
    check(!strided_collision_plan.ok &&
              std::any_of(
                  strided_collision_plan.diagnostics.begin(),
                  strided_collision_plan.diagnostics.end(),
                  [](const ModResolution::Diagnostic& diagnostic) {
                      return diagnostic.resource == "disc_raw:0xcd-0xce";
                  }),
          "strided collision diagnostics must identify the exact owned byte");

    auto reject_strided = [&](const std::string& name,
                              const std::string& layout,
                              uint64_t offset) {
        const fs::path dir = root / "bad-strided" / name;
        write_bytes(dir / "payload.bin", strided_payload);
        write_text(dir / "manifest.toml",
            "format_version = 6\n"
            "id = \"bad." + name + "\"\n"
            "version = \"1.0.0\"\n"
            "name = \"Bad\"\n"
            ...
            layout);
        ModPackage parsed;
        return !ModPackageManager::read_manifest(
            dir / "manifest.toml", parsed, &error);
    };
    check(reject_strided("missing-count",
                         "chunk_size=2\nstride=4\n", 0),
          "strided layouts must declare all three layout fields");
    check(reject_strided("bad-product",
                         "chunk_size=2\nstride=4\nchunk_count=2\n", 0),
          "strided layouts must exactly cover their authored payload");
    check(reject_strided("overlap",
                         "chunk_size=3\nstride=2\nchunk_count=2\n", 0),
          "strided chunks must not overlap each other");
    check(reject_strided(
              "lba-overflow",
              "chunk_size=2\nstride=2\nchunk_count=3\n",
              (uint64_t{1} << 32) * 2352u),
          "raw overlays must stay within 32-bit CD LBA addressability");
    {
        const fs::path path = root / "disc-patch-lba-overflow.toml";
        write_text(
            path,
            manifest(
                "bad.disc-patch-lba", "1.0.0",
                "[[feature]]\n"
                "id=\"bad\"\n"
                "name=\"Bad\"\n"
                "[[patch]]\n"
                ...
                "target=\"disc_raw\"\n"
                "offset=" +
                    std::to_string((uint64_t{1} << 32) * 2352u) +
                    "\nexpected=\"00\"\nreplace=\"01\"\n"));
        ModPackage parsed;
        check(!ModPackageManager::read_manifest(path, parsed, &error),
              "disc patches must stay within 32-bit CD LBA addressability");
    }
