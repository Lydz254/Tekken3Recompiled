...
int main() {
    ...
    std::vector<uint8_t> stock(15 * 2352, 0);
    /* Nonzero sibling/gap sectors prove the strided expected hash excludes
     * bytes the overlay does not own. */
    std::fill(stock.begin() + 9 * 2352, stock.begin() + 10 * 2352, 0x6d);
    std::fill(stock.begin() + 11 * 2352, stock.begin() + 12 * 2352, 0x7e);
    std::vector<uint8_t> overlay(3000);
    for (size_t i = 0; i < overlay.size(); ++i)
        overlay[i] = (uint8_t)(i * 17u + 3u);
    std::vector<uint8_t> strided_overlay(3 * 2336);
    for (size_t i = 0; i < strided_overlay.size(); ++i)
        strided_overlay[i] = (uint8_t)(i * 29u + 5u);
    ...
    write_bytes(root / "packages/runtime.test/1.0.0/assets/overlay.bin",
                overlay);
    write_bytes(root / "packages/runtime.test/1.0.0/assets/strided.bin",
                strided_overlay);
    write_text(root / "packages/runtime.test/1.0.0/manifest.toml",
        ...
        "[[overlay]]\n"
        "feature = \"strided-overlay\"\n"
        "target = \"disc_raw\"\n"
        "offset = 18832\n"
        "file = \"assets/strided.bin\"\n"
        "sha256 = \"" + sha256_hex(strided_overlay) + "\"\n"
        "expected_sha256 = \"" +
            sha256_hex(std::vector<uint8_t>(strided_overlay.size(), 0)) + "\"\n"
        "chunk_size = 2336\n"
        "stride = 4704\n"
        "chunk_count = 3\n"
        "[[plugin]]\n"
    ...
    std::array<uint8_t, 2352> strided_sector{};
    strided_sector.fill(0xcc);
    mod_runtime_patch_disc_sector(
        8, 1, strided_sector.data(), (uint32_t)strided_sector.size());
    check(strided_sector[0] == 0xcc && strided_sector[15] == 0xcc &&
              strided_sector[16] == strided_overlay[0] &&
              strided_sector[2351] == strided_overlay[2335],
          "strided raw overlay must preserve the absolute sector header and "
          "map its first authored chunk");
    strided_sector.fill(0xcc);
    mod_runtime_patch_disc_sector(
        9, 1, strided_sector.data(), (uint32_t)strided_sector.size());
    check(std::all_of(strided_sector.begin(), strided_sector.end(),
                      [](uint8_t value) { return value == 0xcc; }),
          "strided raw overlay must leave sibling/gap sectors untouched");
    strided_sector.fill(0xcc);
    mod_runtime_patch_disc_sector(
        10, 1, strided_sector.data(), (uint32_t)strided_sector.size());
    check(strided_sector[16] == strided_overlay[2336] &&
              strided_sector[2351] == strided_overlay[4671],
          "strided raw overlay must map later chunks at the declared stride");
