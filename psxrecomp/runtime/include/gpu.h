...
/* Optional second packet arena for games that double-buffer HUD primitives. */
...
/* True only for the current/just-completed frame when Tekken 3's exact
 * selector DMA fingerprint was observed. Such a frame has a complete
 * re-laid-out sidecar and must not be overwritten by the canonical splice. */
int  gpu_ws_selector_sidecar_authoritative(void);
/* Completed full-mirror gameplay retained through a no-draw scene/CD wait. */
...
/* Native-wide 2D edge fill: extend only the synthetic margins from a bounded
 * edge-color average after three GTE-quiet frames. The canonical centre is
 * untouched; any 3D projection or MDEC/depth24 frame disables it. */
...
/* Record an exact polygon packet emitted by a title-configured background
 * finalizer. The renderer uses this provenance for background-only edge fill. */
