...
/* Force this primitive's sidecar copy to expand from the authored 4:3 width
 * to the configured native-wide width. Canonical VRAM remains unchanged. */
...
/* Per-present centre ownership. Off/default keeps canonical upload/copy
 * content authoritative; on lets a proven complete wide mirror own gameplay. */
void gr_wide_set_mirror_authoritative(int authoritative);
/* Proves this frame's sidecar contains every centre primitive even when the
 * generic GL fast mode is enabled. This is stricter than authority and must be
 * granted only by a complete scene-specific render path. */
...
typedef struct GpuRenderBackend {
    ...
    void (*wide_set_mirror_authoritative)(int authoritative);
    void (*wide_set_sidecar_complete)(int complete);
