...
struct ModOverlay {
    ...
    uint64_t size = 0;
    /* Format 6 strided overlays map each consecutive payload chunk at
     * location + index * stride.  Ordinary overlays are normalized to one
     * chunk whose size and stride both equal `size`. */
