...
/* Tekken 3's selector is an authored 2D scene, not a camera. The read-only DMA
 * prepass fingerprints its alternating packet arena and records complete
 * packet bounds. The execution path then selects one rigid sidecar delta per
 * command; guest coordinates/canonical VRAM are never rewritten. */
#define T3_SELECTOR_PREPASS_MAX 1024u
typedef struct T3SelectorPrepassItem {
    Tekken3SelectorPacket packet;
} T3SelectorPrepassItem;
static T3SelectorPrepassItem t3_selector_prepass[T3_SELECTOR_PREPASS_MAX];
...
int gpu_tekken3_selector_active(void) {
    return t3_selector_session_frame.active &&
           t3_selector_session_frame.layout_kind == 0;
}
/* Current command may be canonically outside Tekken's retained lower draw
 * area but still needs its exact selector-only sidecar copy. */
static int t3_selector_sidecar_unclipped;

static void t3_selector_forget_frame(void) {
    memset(&t3_selector_frame, 0, sizeof(t3_selector_frame));
    memset(&t3_selector_session_frame, 0, sizeof(t3_selector_session_frame));
}
...
static int ws_nw_2d_edge_fill = 0;
void gpu_ws_set_full_2d(int on) { ws_full_2d = on ? 1 : 0; }
void gpu_ws_set_nw_2d_edge_fill(int on) {
    ws_nw_2d_edge_fill = on ? 1 : 0;
}
...
static int ws_nw_full_mirror = 0;
...
void gpu_ws_set_nw_full_mirror(int on) { ws_nw_full_mirror = on ? 1 : 0; }
int gpu_ws_nw_background_history_enabled(void) {
    return ws_clear_reveal && ws_nw_full_mirror;
}
...
static uint32_t ws_last_any_gte_stamp = (uint32_t)-1000;
...
static int t3_retained_gameplay_active(void) {
    if (!t3_retained_gameplay) return 0;
    GpuDisplayInfo di;
    gpu_get_display_info(&di);
    t3_retained_gameplay = tekken3_gameplay_retain(t3_retained_gameplay,
        ws_mode == 2 && ws_nw_full_mirror && ws_clear_reveal, 0, 0, 0,
        (int32_t)di.width, (int32_t)di.height,
        di.depth24 || mdec_recently_active(WS_FMV_HYSTERESIS));
    return t3_retained_gameplay;
}

int gpu_ws_present_native_43(void) {
    if (!ws_engaged()) return 0;
    /* Tekken 3's selector is a deliberately complete 2D sidecar scene.  Its
     * packet fingerprint is stronger than the generic gameplay detector, so
     * let it opt into native-wide even when ordinary 2D menus are configured
     * to stay on the inexpensive canonical 4:3 path. */
    if (t3_loading_frame.active || t3_selector_session_frame.active) return 0;
    /* Check retained scanout only on the quiet path. Active gameplay keeps
     * its existing cheap per-primitive mode check and cached movie test. */
    if (!ws_game_mode() || ws_2d_only_scene())
        return t3_retained_gameplay_active() ? 0 : 1;
    uint32_t f = (uint32_t)s_frame_count;
...
int gpu_ws_nw_2d_edge_fill_active(void) {
    if (!ws_nw_2d_edge_fill) return 0;
    GpuDisplayInfo di;
    gpu_get_display_info(&di);
    uint32_t age = (uint32_t)s_frame_count - ws_last_any_gte_stamp;
    return psx_ws_2d_edge_fill_allowed(
        1, ws_native_wide_active(), di.depth24,
        mdec_recently_active(WS_FMV_HYSTERESIS), age);
}
...
static int ws_background_guard_pixels = 0;
...
void gpu_ws_set_angle_sites(const uint32_t *addresses,
                            const uint32_t *expected, const uint32_t *full_angles, int nsites) {
    if (nsites < 0) nsites = 0;
    if (nsites > WS_EXPLICIT_CULL_SITES_MAX)
        nsites = WS_EXPLICIT_CULL_SITES_MAX;
    ws_angle_n = nsites;
    for (int i = 0; i < nsites; i++) {
        ...
        ws_angle_sites[i].full_angle = full_angles ? full_angles[i] : 0u;
    }
    ws_angle_calls = 0;
...
uint32_t psx_ws_full_angle_widen(uint32_t vanilla) {
    ws_angle_calls++;
    const int margin = psx_ws_x_margin();
    if (margin <= 0) {
        ws_angle_43_identity++;
    ...
    const uint32_t widened =
        psx_ws_widen_full_angle_q12(vanilla, margin, ws_disp_w());
    if (vanilla > ws_angle_max_vanilla)
        ws_angle_max_vanilla = vanilla;
    if (widened > ws_angle_max_widened)
        ws_angle_max_widened = widened;
...
int psx_ws_angle_site(uint32_t pc, uint32_t instr, uint32_t *out) {
    ...
    for (int i = 0; i < ws_angle_n; i++) {
        ...
        if (site->address != phys || site->expected != instr) continue;
        if (out)
            *out = (site->full_angle ? psx_ws_full_angle_widen : psx_ws_angle_widen)(
...
void gpu_ws_set_background_guard_pixels(int pixels) {
    if (pixels < 0) pixels = 0;
    if (pixels > 32768) pixels = 32768;
    ws_background_guard_pixels = pixels;
}

uint32_t psx_ws_shell_source_index(uint32_t index, uint32_t base_cols,
                                   uint32_t wide_cols) {
    if (psx_ws_x_margin() <= 0 || base_cols == 0u || wide_cols <= base_cols)
        return index;

    const uint32_t border = (wide_cols - base_cols) / 2u;
    const uint32_t row = index / wide_cols;
    const uint32_t col = index % wide_cols;

    /* Wrap the newly prepended/appended border across the stock finite grid.
     * For 6->8 this maps each axis [0..7] to [5,0,1,2,3,4,5,0]. The world
     * coordinates remain [-35..35]; only the descriptor lookup is remapped,
     * so every added cell is real geometry backed by an existing valid tile. */
    const uint32_t source_row =
        (row + base_cols - (border % base_cols)) % base_cols;
    const uint32_t source_col =
        (col + base_cols - (border % base_cols)) % base_cols;
    return source_row * base_cols + source_col;
}
...
int psx_ws_background_cull_margin(void) {
    const int margin = psx_ws_x_margin();
    return margin > 0 ? margin + ws_background_guard_pixels : 0;
}

int psx_ws_background_cull_bltz(uint32_t v) {
    return ((int32_t)v < -psx_ws_background_cull_margin()) ? 1 : 0;
}
...
typedef struct {
    uint32_t key, stamp;
    uint32_t sig[4];
} WsBackgroundPacketTag;
#define WS_BACKGROUND_PACKET_TAGS 2048
static WsBackgroundPacketTag ws_background_packet_tags[WS_BACKGROUND_PACKET_TAGS];

void psx_ws_background_packet_tag(uint32_t addr) {
    if (!ws_engaged()) return;
    uint32_t now = (uint32_t)s_frame_count;
    uint32_t key = addr & 0x1FFFFCu;
    if (!key) return;
    uint32_t idx = (key >> 2) & (WS_BACKGROUND_PACKET_TAGS - 1);
    ws_background_packet_tags[idx].key = key;
    ws_background_packet_tags[idx].stamp = now;
    /* Packet memory is pooled and reused by later actor draws. Address alone
     * therefore is not provenance: after reuse it made Eddy/Lee look like the
     * backdrop. Fingerprint the completed GP0 command behind the packet tag so
     * only the exact mesh produced by this call can match at execution time. */
    for (int i = 0; i < 4; i++)
        ws_background_packet_tags[idx].sig[i] = psx_read_word(addr + 4u + (uint32_t)i * 4u);
}

static int ws_background_packet_is_tagged(void) {
    if (!ws_engaged() || gp0_cmd_source_addr == 0xFFFFFFFFu) return 0;
    /* func_8006CC28 is a shared mesh-packet emitter, not a backdrop-only
     * routine: Tekken calls it for fighters as well.  The jungle trace keeps
     * its FT4/FT3/F4 backdrop in the first OT nodes (ranks 9 and 11), while
     * fighter meshes start much later (rank 54+).  Require the proven early
     * background layer here so pooled actor packets can never enter the
     * retained background sidecar. */
    if (gp0_ot_rank == 0xFFFFu || gp0_ot_rank > 16u) return 0;
    uint32_t now = (uint32_t)s_frame_count;
    for (int variant = 0; variant < 2; variant++) {
        uint32_t key = (gp0_cmd_source_addr - (variant ? 0u : 4u)) & 0x1FFFFCu;
        uint32_t idx = (key >> 2) & (WS_BACKGROUND_PACKET_TAGS - 1);
        WsBackgroundPacketTag *t = &ws_background_packet_tags[idx];
        if (t->key == key && now - t->stamp <= 2 &&
            t->sig[0] == gp0_cmd_buf[0] &&
            t->sig[1] == gp0_cmd_buf[1] &&
            t->sig[2] == gp0_cmd_buf[2] &&
            t->sig[3] == gp0_cmd_buf[3])
            return 1;
    }
    return 0;
}

/* Tekken 3's separately projected stage backdrop is intentionally submitted
 * far outside the canonical viewport so native-wide can reveal it.  That is
 * safe until the gameplay camera pulls through one of the finite backdrop
 * faces: the PS1 GTE then clamps vertices to both Screen-X saturation rails
 * (-1024 and +1023).  The stock 4:3 all-left/all-right tests cannot reject a
 * polygon spanning both rails, and the wide edge expansion turns it into the
 * characteristic screen-high repeated-texture smear.
 *
 * Packet provenance is essential here.  Applying this test to arbitrary GTE
 * geometry could remove legitimate large floor/world polygons.  The three
 * packet-tag sites in func_8006CC28 identify only Tekken's FT4, FT3 and F4
 * background packets, so reject just those impossible both-rail projections.
 * A valid off-screen backdrop may touch either rail by itself and is retained.
 */
static int ws_background_packet_projection_overflow(uint8_t opcode) {
    if (!ws_background_packet_is_tagged()) return 0;

    int indices[4];
    int count = 0;
    if (opcode >= 0x2Cu && opcode <= 0x2Fu) {       /* FT4 */
        indices[0] = 1; indices[1] = 3;
        indices[2] = 5; indices[3] = 7;
        count = 4;
    } else if (opcode >= 0x24u && opcode <= 0x27u) { /* FT3 */
        indices[0] = 1; indices[1] = 3; indices[2] = 5;
        count = 3;
    } else if (opcode >= 0x28u && opcode <= 0x2Bu) { /* F4 */
    ...
    int hit_left_rail = 0;
    int hit_right_rail = 0;
    for (int i = 0; i < count; i++) {
        int32_t x, y;
        parse_vertex(gp0_cmd_buf[indices[i]], &x, &y);
        (void)y;
        if (x <= -1024) hit_left_rail = 1;
        if (x >=  1023) hit_right_rail = 1;
    }
    return hit_left_rail && hit_right_rail;
}
static int ws_nw_phase_backdrop = 0;
void gpu_ws_set_nw_phase_backdrop(int on) { ws_nw_phase_backdrop = on ? 1 : 0; }
static int ws_nw_ui_stretch_min_ot = 0;
void gpu_ws_set_nw_ui_stretch_min_ot(int min_ot) {
    ws_nw_ui_stretch_min_ot = min_ot > 0 ? min_ot : 0;
}
...
static int ws_nw_backdrop_current_quad(void);
...
int psx_ws_prim_in_backdrop(void) {
    ...
    /* Tagged stage faces already use the widened guest projection/cull.
     * A second, edge-only screen-space expansion breaks shared vertices as
     * they cross the 4:3 boundary during rotation. Keep their projected X. */
    if (ws_background_packet_is_tagged()) return 0;
    /* Strict full-frame quad path ([widescreen] nw_backdrop).  Unlike the
     * historical implementation, this only classifies the current command;
     * the GL/SW wide mirror applies the scale while the canonical draw remains
     * byte-identical.  Complete GTE-projected world/fighter geometry cannot
     * match the provenance-first backdrop policy. */
    if (ws_nw_backdrop_current_quad()) return 1;
    ...
    /* Tekken 3 submits its complete practice/battle UI as a final rectangle/
     * sprite layer (OT 77 in the jungle trace; the 3D world ends at OT 68).
     * Stretch only that proven front layer in the synthetic wide target. The
     * canonical framebuffer, GTE camera, fighters, and polygonal stage remain
     * untouched. */
    if (ws_nw_ui_stretch_min_ot > 0 && ws_bg_phase_over() &&
        gp0_ot_rank >= (uint16_t)ws_nw_ui_stretch_min_ot) {
        uint32_t op = (gp0_cmd_buf[0] >> 24) & 0xFFu;
        if (op >= 0x60u && op <= 0x7Fu) return 1;
    }
 ...
 * zone share a shift and stay aligned. Configured command-source ranges let
 * pure-2D games opt in only their HUD packet arenas (including alternate-frame
 * buffers), leaving world primitives untouched. Identity unless native-wide is
 * engaged AND the game opts in, so
...
static uint32_t ws_nw_hud_packet2_lo = 0;
static uint32_t ws_nw_hud_packet2_hi = 0;
...
void gpu_ws_set_nw_hud_packet2_range(uint32_t lo, uint32_t hi) {
    ws_nw_hud_packet2_lo = lo & 0x1FFFFFFFu;
    ws_nw_hud_packet2_hi = hi & 0x1FFFFFFFu;
}
static int ws_nw_hud_packet(void) {
    return psx_ws_hud_packet_range_match(
        ws_native_wide_active(), gp0_cmd_source_addr,
        ws_nw_left_hud_packet_lo, ws_nw_left_hud_packet_hi,
        ws_nw_hud_packet2_lo, ws_nw_hud_packet2_hi);
}
static int32_t ws_nw_hud_shift(int32_t x, int32_t w) {
    if (!ws_native_wide_active()) return 0;
    int32_t off = ws_nw_offset();
    if (off <= 0) return 0;
    if (!ws_nw_hud_packet() && !ws_nw_hud_corners) return 0;
 ...
 * world quads are excluded by exact RAM provenance before shape is considered.
 * This helper is classification-only: the canonical vertices never change;
 * GL/SW apply the scale exclusively to their synthetic wide target. */
...
static int ws_nw_backdrop_quad(const int32_t *vx, const int32_t *vy,
                               uint32_t op, const int *word_indices) {
    if (!ws_nw_backdrop || !ws_native_wide_active()) return 0;
    if (ws_nw_extra() <= 0 || !vx || !vy || !word_indices) return 0;
    int32_t W = ws_disp_w();
    ...
    if (!ws_axis_aligned_quad(vx, vy)) return 0;

    /* Exact GTE-to-DMA provenance owns world classification.  Fail closed if
     * every quad position is a live SWC2 projection store: neither a fighter
     * nor a stage mesh may become a screen-space backdrop based on shape. */
    uint8_t gte_mask = 0;
    if (gp0_cmd_source_addr != 0xFFFFFFFFu) {
        for (int i = 0; i < 4; i++) {
            int wi = word_indices[i];
            uint32_t addr = (gp0_cmd_source_addr + (uint32_t)wi * 4u) & 0x1FFFFCu;
            if (gte_precision_load_word(addr, gp0_cmd_buf[wi], NULL, NULL, NULL))
                gte_mask |= (uint8_t)(1u << i);
        }
    }
    ...
    e.opcode = (uint8_t)op;
    e.vertex_count = 4;
    e.gte_position_mask = gte_mask;
    e.sprite_tagged = (uint8_t)(psx_ws_prim_is_tagged() ? 1 : 0);
    e.axis_aligned = 1;
    e.scene_has_3d = 1;
    e.world_seen_before = 0;
    ...
    e.max_y = maxy;
    e.display_width = W;
    e.display_height = ws_disp_h();
    return psx_ws_is_opaque_backdrop_band(&e);
}
...
static int ws_nw_backdrop_current_quad(void) {
    ...
    if (op >= 0x28u && op <= 0x2Fu) {
        wi[0] = 1; wi[1] = (op & 0x04u) ? 3 : 2;
        wi[2] = (op & 0x04u) ? 5 : 3; wi[3] = (op & 0x04u) ? 7 : 4;
    } else if (op >= 0x38u && op <= 0x3Fu) {
        wi[0] = 1; wi[1] = (op & 0x04u) ? 4 : 3;
        wi[2] = (op & 0x04u) ? 7 : 5; wi[3] = (op & 0x04u) ? 10 : 7;
    } else {
        return 0;
    }
    int32_t vx[4], vy[4];
    for (int i = 0; i < 4; i++) parse_vertex(gp0_cmd_buf[wi[i]], &vx[i], &vy[i]);
    return ws_nw_backdrop_quad(vx, vy, op, wi);
}
...
static int ws_fb_world_mode[WS_FB_BASES];
static uint32_t ws_fb_margin_frame[WS_FB_BASES] = {
...
static void ws_note_display_base(uint32_t bx) {
    ...
    if (ws_fb_n < WS_FB_BASES) {
        ws_fb_base[ws_fb_n] = bx;
        ws_fb_world_mode[ws_fb_n] = 0;
        ws_fb_margin_frame[ws_fb_n] = UINT32_MAX;
        ws_fb_n++;
        return;
    }
    for (int i = 1; i < WS_FB_BASES; i++) {
        ws_fb_base[i - 1] = ws_fb_base[i];
        ws_fb_world_mode[i - 1] = ws_fb_world_mode[i];
        ws_fb_margin_frame[i - 1] = ws_fb_margin_frame[i];
    }
    ws_fb_base[WS_FB_BASES - 1] = bx;
    ws_fb_world_mode[WS_FB_BASES - 1] = 0;
    ws_fb_margin_frame[WS_FB_BASES - 1] = UINT32_MAX;
}
...
/* A CPU upload / nontrivial VRAM copy can replace the canonical picture
 * without drawing its sidecar. Texture-only uploads and self-copies do not
 * end a retained gameplay image. Wrapped destinations conservatively end it. */
static void t3_note_framebuffer_write(uint32_t x, uint32_t y,
                                      uint32_t w, uint32_t h) {
    if (!t3_retained_gameplay || !w || !h) return;
    if (x + w > 1024u || y + h > 512u) {
        t3_retained_gameplay = 0;
        return;
    }
    if (y >= 480u) return;
    for (int i = 0; i < ws_fb_n; i++) {
        if (x < ws_fb_base[i] + 368u && x + w > ws_fb_base[i]) {
            t3_retained_gameplay = 0;
            return;
        }
    }
...
static void ws_nw_sync_target(void) {
    ...
    /* Main-menu text scrolls inside x=48..367, y=316..431. E3 narrows
     * the existing framebuffer clip; it does not select a buffer at x=48.
     * Keep the proven menu on its original target and retain its Y clip. */
    if (t3_selector_session_frame.active &&
        t3_selector_session_frame.layout_kind == 1 &&
        base == 48u && draw_area_right == 367u)
        base = 0;
...
static void ws_refresh_current_reveal_margins(void) {
    /* Most PSX 3D stages retain chunks of their backdrop in the framebuffer
     * and redraw them only when the camera exposes a new strip.  Clearing the
     * host-owned columns every frame turns that normal retention into visible
     * 4:3 popping.  Trail-prevention therefore remains an explicit per-title
     * opt-in instead of silently destroying persistent stage pixels. */
    if (!ws_clear_reveal || !ws_native_wide_active()) return;
    /* Full-sidecar frames may redraw only a strip (Tekken's pause menu).
     * Wait until the first primitive has the final E3/E4 environment, and
     * preserve pixels outside that guest-authored update region. */
    if (draw_area_right < draw_area_left || draw_area_bottom < draw_area_top ||
        draw_area_top >= 512u) return;
    uint32_t base = draw_area_left;
    for (int i = 0; i < ws_fb_n; i++) {
        if (ws_fb_base[i] != base) continue;
        if (!psx_ws_margin_refresh_once((uint32_t)s_frame_count,
                                        &ws_fb_margin_frame[i]))
            return;
        const int world_mode = ws_game_mode();
        const int world_entry = world_mode && !ws_fb_world_mode[i];
        ws_fb_world_mode[i] = world_mode;
        /* Compose an authoritative sidecar from the current camera frame.
         * Screen-space background history retains the old view's silhouettes
         * wherever the new view has no polygon coverage. */
        if (ws_nw_full_mirror && world_entry)
            /* Gameplay cannot retain the previous menu's artwork, including
             * rows outside its letterboxed draw area. Later partial updates
             * (pause) preserve the current stage below their clip region. */
            gr_wide_clear((int)base, 0, 512, 0);
        else if (ws_nw_full_mirror)
            gr_wide_clear((int)base, (int)draw_area_top,
                (int)(draw_area_bottom < 512u ? draw_area_bottom + 1u : 512u) -
                    (int)draw_area_top, 0);
        else
            gr_wide_clear_margins((int)base, 0, 512, 0, 3);
        return;
    }
}

/* A native-wide sidecar is host-owned and is not automatically redrawn like
 * the guest's canonical framebuffer. Refresh its synthetic columns once per
 * active back buffer and frame, before this list submits any margin geometry. */
void gpu_ws_begin_linked_list(void) {
    gp0_ot_rank = 0xFFFFu;
    ws_nw_sync_target();
    if (!ws_nw_full_mirror) ws_refresh_current_reveal_margins();
}
...
static void gpu_reset_state(int clear_vram) {
    ...
    memset(&t3_loading_frame, 0, sizeof(t3_loading_frame));
    t3_force_reveal_count = 0;
    t3_force_background_active = 0;
    if (clear_vram) {
    ...
    for (int i = 0; i < WS_FB_BASES; i++)
        ws_fb_margin_frame[i] = UINT32_MAX;
    polyline_color = 0;
...
static void gp0_exec_shaded_quad(void) {
    ...
    /* The selector retains its red roster gradient below the upper-panel
     * clip (y<=305 unlocked). Its native-wide copy must be reconstructed even
     * when canonical VRAM keeps the old pixels, like the roster SPRTs. The
     * backend still clips the canonical draw to the guest area. */
    if (draw_area_out_bbox(vx, vy, 4) && !t3_selector_sidecar_unclipped) return;
 ...
 * ordinary renderer clip keeps canonical VRAM exact, while the native-wide
 * mirror receives the correctly indexed map tiles at the original draw depth. */
static void t3_force_draw_reveal(uint32_t source, uint32_t color24, int semi, int raw) {
    for (size_t i = 0; i < t3_force_reveal_count; i++) {
        const Tekken3ForceReveal *r = &t3_force_reveals[i];
        if (r->edge_source != (source & 0x1fffffu)) continue;
        setup_textured_draw(color24, semi, raw);
        gr_draw_textured_rect(r->x + draw_offset_x, r->y + draw_offset_y, 64, 64,
...
static void gp0_exec_textured_rect(void) {
    ...
    if (w > 1023) w = 1023;
    if (h > 511)  h = 511;
    if (t3_force_reveal_count && w == 64 && h == 64)
        t3_force_draw_reveal(gp0_cmd_source_addr, color24, semi_trans, raw_texture);
    ...
    x0 += draw_offset_x; y0 += draw_offset_y;
    {
        ...
        if (draw_area_out_rect(x0, y0, dw, h) &&
            !t3_selector_sidecar_unclipped)
            return;
    }
    setup_textured_draw(color24, semi_trans, raw_texture);
    if (ws_w && ws_w != w)
...
static void gp0_exec_fill_rect(void) {
    ...
    } else t3_note_framebuffer_write(dst_x, dst_y, width, height);
}
...
static void gp0_exec_draw_area_tl(void) {
    ...
    /* E3 can switch framebuffer after linked-list begin. The per-base/frame
     * stamp makes this free when begin already selected the same target, while
     * guaranteeing the new target is clean before the first following draw. */
    if (!ws_nw_full_mirror) ws_refresh_current_reveal_margins();
}
...
static void gp0_exec_cpu_to_vram(void) {
    ...
    t3_note_framebuffer_write(vram_write_x, vram_write_y,
                             vram_write_w, vram_write_h);
...
static int ws_prepass_packet_bounds(const uint32_t *words,
                                    Tekken3SelectorPacket *out) {
    ...
    } else if (op == 0x40u || op == 0x42u) {
        /* Team roster bevels are independent inclusive flat lines. Include
         * one-pixel horizontal/vertical lines in the authored UI prepass. */
        int32_t x0, y0, x1, y1;
        parse_vertex(words[1], &x0, &y0);
        parse_vertex(words[2], &x1, &y1);
        min_x = x0 < x1 ? x0 : x1;
        min_y = y0 < y1 ? y0 : y1;
        max_x = (x0 > x1 ? x0 : x1) + 1;
        max_y = (y0 > y1 ? y0 : y1) + 1;
    } else if ((op >= 0x60u && op <= 0x67u)) {
        parse_vertex(words[1], &min_x, &min_y);
        int32_t width = (int32_t)(words[op >= 0x64u ? 3 : 2] & 0xFFFFu);
        int32_t height = (int32_t)((words[op >= 0x64u ? 3 : 2] >> 16) & 0xFFFFu);
...
static void t3_selector_prepass_add(const uint32_t *words,
                                    uint32_t source_addr) {
    if (t3_selector_prepass_count >= T3_SELECTOR_PREPASS_MAX) return;
    Tekken3SelectorPacket packet;
    if (!ws_prepass_packet_bounds(words, &packet)) return;
    packet.source_addr = source_addr & 0x1FFFFCu;
    t3_selector_prepass[t3_selector_prepass_count++].packet = packet;
}
...
/* SLUS-00402 Force overlay: 800B4914 draws seven 64px columns per row.
 * Its 42-slot A/B sprite buffers cannot be enlarged safely. Read the verified
 * tile/palette maps and emit only missing sidecar cells, without guest writes. */
static void t3_force_prepare_reveal(void) {
    t3_force_reveal_count = 0;
    t3_force_background_active = 0;
    if (!t3_selector_prepass_count || !ws_engaged() || ws_mode != 2 ||
        !ws_nw_full_mirror || !ws_game_mode() ||
        ws_disp_w() != 368 || ws_disp_h() != 480 ||
        psx_read_word(0x800B4914u) != 0x27bdff50u ||
        psx_read_word(0x800B4CB0u) != 0xa622000eu ||
        psx_read_word(0x800B4E98u) != 0x2ae20007u) return;
    t3_force_background_active = 1;
    unsigned angle = psx_read_word(0x800B6BACu) & 0xfffu;
    int cosine = (int16_t)psx_read_half(0x8001F0C4u+angle*2u);
    ...
    uint32_t tiles = psx_read_word(0x800B6B9Cu), palettes = psx_read_word(0x800B6BA0u);
    uint32_t base = psx_read_word(0x800ADD58u);
    if (columns < 1 || columns > T3_FORCE_MAX_COLUMNS || rows < 1 ||
        rows > T3_FORCE_MAX_ROWS || divisor <= 0 ||
        (tiles & 0xffe00001u) != 0x80000000u ||
        (palettes & 0xffe00001u) != 0x80000000u ||
        (base & 0xffe00003u) != 0x80000000u ||
        (tiles & 0x1fffffu) + (unsigned)(columns*rows*2) > 0x200000u ||
        (palettes & 0x1fffffu) + (unsigned)(columns*rows*2) > 0x200000u ||
        (base & 0x1fffffu) + 2352u > 0x200000u) return;
    Tekken3ForceCell map[T3_FORCE_MAX_COLUMNS*T3_FORCE_MAX_ROWS];
    for (int i = 0; i < columns*rows; i++) {
        map[i].tile = psx_read_half(tiles + (uint32_t)i*2);
        map[i].clut = psx_read_half(palettes + (uint32_t)i*2);
    }
    for (unsigned buffer = 0; buffer < 2; buffer++) {
        Tekken3ForceSprite sprites[42];
        size_t count = 0;
        uint32_t first = (base & 0x1fffffu) + buffer*840u + 4u;
        for (size_t i = 0; i < t3_selector_prepass_count && count < 42; i++) {
            const Tekken3SelectorPacket *p = &t3_selector_prepass[i].packet;
            uint32_t src = p->source_addr & 0x1fffffu;
            if (p->opcode != 0x64 || p->width != 64 || p->height != 64 ||
                src < first || src >= first+840u || (src-first)%20u) continue;
            unsigned index = (src-first)/20u;
            uint32_t uv = psx_read_word(src+8u);
            uint32_t page = psx_read_word(base+1680u+buffer*336u+index*8u+4u);
            if ((uv & 0x3f3fu) || (page & 0xffffffe0u) != 0xe1000200u) continue;
            sprites[count++] = (Tekken3ForceSprite){src, p->x, p->y,
                {(uint16_t)(((uv & 255u)/64u) | (((uv >> 8)&255u)/64u)<<2 |
                    ((page & 31u)<<8)), (uint16_t)(uv >> 16)}};
        }
        int scroll = (int32_t)psx_read_word(0x800B6C0Cu) / divisor;
        t3_force_reveal_count += tekken3_force_reveal(map, columns, rows, scroll, (int)slope,
...
void gpu_ws_prepass_linked_list(uint32_t start_addr) {
    int t3_selector_started = 0;
    int t3_loading_started = 0;
    t3_force_reveal_count = 0;
    t3_force_background_active = 0;
    int t3_loading_foreign_draw = 0;
    int has_draw = 0;
    ws_ui_prepass_count = 0;
    ws_ui_prepass_rank = 0xFFFFu;
    ws_auto_ui_dense = 0;
    ...
    for (;;) {
        ...
        if (num_words == 0) {
            ...
            while (offset < num_words) {
                ...
                if (op >= 0x20u && op < 0x80u) has_draw = 1;
                ...
                uint32_t command_source = psx_mod_gpu_dma_resolve_address(
                    word_addr + offset * 4u);
                if (t3_loading_frame.active && op >= 0x20u && op < 0x80u &&
                    ((command_source & 0x1FFFFFu) < t3_loading_frame.arena_lo ||
                     (command_source & 0x1FFFFFu) >= t3_loading_frame.arena_hi))
                    t3_loading_foreign_draw = 1;
                t3_selector_prepass_add(words, command_source);
                if (ws_auto_ui_squash && ws_active())
                    ws_ui_prepass_add(words, command_source, rank);
    ...
    {
        /* State flags change before the CD wait ends. A no-draw list still
         * displays the completed stage, regardless of time or game_mode.
         * Any newly drawn non-game scene releases it before execution. */
        t3_retained_gameplay = tekken3_gameplay_retain(t3_retained_gameplay,
            ws_mode == 2 && ws_nw_full_mirror && ws_clear_reveal,
            has_draw, ws_game_mode() && !ws_2d_only_scene(), 0,
            (int32_t)ws_disp_w(), (int32_t)ws_disp_h(),
            display_depth || mdec_recently_active(WS_FMV_HYSTERESIS));
        Tekken3SelectorPacket packets[T3_SELECTOR_PREPASS_MAX];
        for (uint32_t i = 0; i < t3_selector_prepass_count; i++) {
            packets[i] = t3_selector_prepass[i].packet;
        }
        t3_force_prepare_reveal();
        Tekken3LoadingFrame loading;
        if (tekken3_loading_analyze_frame(packets, t3_selector_prepass_count,
                ws_disp_w(), ws_disp_h(), &loading)) {
            t3_loading_started = !t3_loading_frame.active ||
                t3_loading_frame.layout_kind != loading.layout_kind;
            t3_loading_frame = loading;
            t3_selector_forget_frame();
        } else if (t3_loading_foreign_draw || ws_disp_w() != 368) {
            /* CD/texture uploads can last hundreds of frames without a draw.
             * Keep the completed versus image until another scene draws. */
            memset(&t3_loading_frame, 0, sizeof(t3_loading_frame));
        }
        (void)tekken3_selector_analyze_frame(
            packets, t3_selector_prepass_count,
            ws_disp_w(), ws_disp_h(), &t3_selector_frame);
        if (t3_selector_frame.active) {
            /* The lower cabinet and roster are intentionally retained in the
             * framebuffer.  A selector frame can clamp its draw area to y=339
             * after the initial full-height pass, so later frames redraw the
             * upper composition and cursor without resubmitting those static
             * lower sprites.  Remember whether this is the first complete
             * selector frame before updating the retained composition. */
            t3_selector_started = !t3_selector_session_frame.active ||
                t3_selector_session_frame.layout_kind != t3_selector_frame.layout_kind;
            t3_selector_session_frame = t3_selector_frame;
            memset(&t3_loading_frame, 0, sizeof(t3_loading_frame));
        } else if (tekken3_selector_can_retain_frame(&t3_selector_session_frame,
                       packets, t3_selector_prepass_count,
                       ws_disp_w(), ws_disp_h())) {
            /* Confirming a character leaves the completed menu on screen
             * during a no-draw/CD wait. Time passing cannot revoke ownership:
             * it would expose the original 4:3 framebuffer for those frames.
             * Same-arena cursor updates retain it too; new scene draws end it. */
            t3_selector_frame = t3_selector_session_frame;
        } else {
            t3_selector_forget_frame();
        }
    }
    /* The exact selector fingerprint proves this 2D frame is completely
     * represented in the sidecar. Disable GL's centre-only batch elision now,
     * before any packets from the list are submitted. */
    if (ws_engaged()) {
        int selector_complete =
            t3_loading_frame.active || t3_selector_session_frame.active ||
            t3_retained_gameplay_active();
        /* The list began on the generic 4:3 menu path.  Once its complete
         * selector fingerprint is known, arm the wide target before the first
         * packet is executed rather than waiting one frame. */
        if (selector_complete) ws_nw_sync_target();
        /* Allocating a new target resets backend authority. Set the contract
         * after sync, including lists that only update a retained UI frame. */
        gr_wide_set_sidecar_complete(selector_complete);
        gr_wide_set_mirror_authoritative(selector_complete);
        /* Selector presentation is complete-sidecar and must never inherit a
         * strip of the gameplay backdrop that occupied this back buffer on the
         * preceding scene. Clear once on selector entry. Do not clear every
         * selector frame: the guest retains its static lower cabinet/portrait
         * pixels while later y<=339 passes redraw only the upper composition. */
        if (selector_complete && (t3_selector_started || t3_loading_started))
            gr_wide_clear((int)draw_area_left, 0, 512, 0);
    }
    if (!ws_engaged() || ws_ui_prepass_count == 0) {
        ws_ui_prepass_count = 0;
        return;
    }
...
static void gp0_execute_command(void) {
    ...
    /* Do this after the complete GP0 packet has been collected, but before it
     * reaches either the canonical renderer or the native-wide sidecar. */
    if (opcode >= 0x20u && opcode <= 0x3Fu &&
        ws_background_packet_projection_overflow(opcode)) {
        return;
    }
    if (opcode >= 0x20u && opcode <= 0x7Fu) {
        /* Default identity for every DRAW. Environment commands and NOPs emit
         * no geometry, so they intentionally retain the pending delta until
        ...
        if ((t3_selector_frame.active || t3_loading_frame.active) &&
            gp0_cmd_source_addr != 0xFFFFFFFFu) {
            Tekken3SelectorPacket packet;
            if (ws_prepass_packet_bounds(gp0_cmd_buf, &packet)) {
                packet.source_addr = gp0_cmd_source_addr & 0x1FFFFCu;
                Tekken3SelectorPlacement placement = t3_loading_frame.active
        ...
        /* The Force overlay also emits a solid sky/underlay at these two
         * fixed packet slots. Extend its authored full-width rectangle. */
        if (t3_force_background_active && opcode == 0x60 &&
            ((gp0_cmd_source_addr & 0x1fffffu) == 0xB6B18u ||
             (gp0_cmd_source_addr & 0x1fffffu) == 0xB6B28u) &&
        ...
        t3_selector_sidecar_unclipped = unclipped_sidecar;
    }
    ...
    switch (opcode) {
        ...
        case 0x9C: case 0x9D: case 0x9E: case 0x9F: {
            ...
            if (src_x != dst_x || src_y != dst_y)
                t3_note_framebuffer_write((uint32_t)dst_x, (uint32_t)dst_y,
                                         (uint32_t)w, (uint32_t)h);
...
static void gp1_display_enable(uint32_t val) {
    ...
    if (next_disabled && !display_disabled && ws_mode == 2) {
        ws_clear_all_reveal_margins();
    }
...
static void gp1_h_display_range(uint32_t val) {
    ...
    /* The range participates in gpu_get_display_info()'s live width. Keep the
     * native-wide sidecar/FBO geometry in lockstep even when a title changes
     * menu/game resolution without immediately reissuing GP0(E3/E4). */
    ws_nw_sync_target();
}
...
static void gp1_display_mode(uint32_t val) {
    ...
    if (display_depth || ws_disp_w() != 368 ||
        (t3_selector_session_frame.active &&
         ws_disp_h() != t3_selector_session_frame.display_height)) {
        t3_selector_forget_frame();
        memset(&t3_loading_frame, 0, sizeof(t3_loading_frame));
        t3_force_reveal_count = 0;
        t3_force_background_active = 0;
    }
    /* hres1/hres2 select 256/320/368/512/640 scanout widths. Resynchronise
     * after both fields are committed so wide geometry is never calculated
     * from a half-updated CRTC mode. In 4:3, ws_nw_sync_target() only disables
     * the synthetic target and therefore preserves exact canonical output. */
    ws_nw_sync_target();
}

int gpu_ws_selector_sidecar_authoritative(void) {
    return ws_native_wide_active() &&
        (t3_loading_frame.active || t3_selector_session_frame.active);
}

int gpu_ws_retained_gameplay_authoritative(void) {
    return t3_retained_gameplay_active() && ws_native_wide_active();
}
...
int gpu_snapshot_read(const uint8_t *p, uint32_t len) {
    ...
    /* UI sidecars are host state, never restored from canonical VRAM. Require
     * a fresh scene fingerprint after loading a guest snapshot. */
    t3_retained_gameplay = 0;
    memset(&t3_loading_frame, 0, sizeof(t3_loading_frame));
    t3_force_reveal_count = 0;
    t3_force_background_active = 0;
    ...
    gr_set_draw_area((int)draw_area_left, (int)draw_area_top,
                     (int)draw_area_right, (int)draw_area_bottom);
    /* Host-owned reveal pixels are not in a guest savestate. Discard the
     * previous scene, including retained rows outside the restored draw area. */
    ws_note_display_base(display_area_x);
    ws_nw_sync_target();
    ws_clear_all_reveal_margins();
    for (int i = 0; i < ws_fb_n; i++)
        ws_fb_margin_frame[i] = UINT32_MAX;
