...
static void handle_outfit_slots(int id,const char *json) {
    ...
    send_fmt("{\"id\":%d,\"ok\":true,\"changed\":%d,\"menu\":%d,\"players\":["
       "{\"visible\":%d,\"character\":%d,\"custom\":%d,\"locked\":%d,\"index\":%d,\"count\":%d,\"name\":\"%s\"},"
       "{\"visible\":%d,\"character\":%d,\"custom\":%d,\"locked\":%d,\"index\":%d,\"count\":%d,\"name\":\"%s\"}]}",
       id,changed,tekken3_outfits_menu_player(),a.visible,a.character,a.custom,a.locked,a.index,a.count,a.name,
       b.visible,b.character,b.custom,b.locked,b.index,b.count,b.name);
}
...
/* ws_camera_zoom num=<n> den=<d>: live-test the native-wide camera pullback
 * without a relaunch. Identity (1/1) is useful for separating finite backdrop
 * coverage from the configured gameplay framing. */
extern void gte_ws_set_camera_zoom(int num, int den);
static void handle_ws_camera_zoom(int id, const char *json)
{
    int num = json_get_int(json, "num", -1);
    int den = json_get_int(json, "den", -1);
    if (num <= 0 || den <= 0) { send_err(id, "need num>0 den>0"); return; }
    gte_ws_set_camera_zoom(num, den);
    send_fmt("{\"id\":%d,\"ok\":true,\"num\":%d,\"den\":%d}", id, num, den);
}

extern void gte_ws_set_dome_camera_y_comp(int on);
static void handle_ws_dome_y_comp(int id, const char *json)
{
    int on = json_get_int(json, "on", -1);
    if (on < 0) { send_err(id, "need on=0 or on=1"); return; }
    gte_ws_set_dome_camera_y_comp(on);
...
static void handle_outfit_screenshot(int id,const char *json) {
    extern int gl_renderer_capture_gallery(uint8_t **rgb,int *width,int *height);
...
static void handle_wide_shot(int id, const char *json)
{
    ...
    /* Debug requests can arrive after an offscreen upload disabled the draw
     * target, before main's present hook restores the UI authority flags.
     * Match that hook here: a canonical center blit would both photograph and
     * overwrite the independently positioned selector/loading composition. */
    if (gpu_ws_selector_sidecar_authoritative()) {
        gr_wide_set_sidecar_complete(1);
        gr_wide_set_mirror_authoritative(1);
    }
...
static const CmdEntry s_commands[] = {
    ...
    { "ws_camera_zoom",    handle_ws_camera_zoom },
    { "ws_dome_y_comp",    handle_ws_dome_y_comp },
