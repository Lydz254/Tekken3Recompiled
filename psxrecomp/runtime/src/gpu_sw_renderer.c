...
/* Off by default: canonical VRAM remains the safe centre source for 2D uploads
 * and VRAM copies. Main enables this only for opted complete-mirror gameplay. */
static int       g_wide_mirror_authoritative = 0;
static int       g_wide_sidecar_complete = 0;
...
static inline RTarget rt_wide(void) {
    ...
    t.cx1 = 0;
    t.cy1 = g_wide_prim_unclipped ? 0 : g_clip_y1 * s;
    t.cx2 = g_wide_w * s - 1;
...
void sw_wide_set_primitive_suppressed(int suppressed) {
    g_wide_prim_suppressed = suppressed ? 1 : 0;
}

void sw_wide_set_primitive_unclipped(int unclipped) {
    g_wide_prim_unclipped = unclipped ? 1 : 0;
}

void sw_wide_set_primitive_expanded(int expanded) {
    g_wide_prim_expanded = expanded ? 1 : 0;
}
...
static inline WideBd wide_bd_get(void) {
    ...
    if (g_wide_w > 0 && g_wide_cur &&
        (g_wide_prim_expanded ||
         (g_ws_bd_stretch_on && psx_ws_prim_in_backdrop()))) {
        int native_w = g_wide_w - 2 * g_wide_off;
        if (native_w > 0) {
            b.on = 1;
            b.scale = g_wide_prim_expanded
...
void sw_draw_flat_triangle(int x0, int y0, int x1, int y1,
    ...
    if (g_wide_cur && !g_wide_prim_suppressed) {
...
/* Copy the authoritative canonical framebuffer into the centre of a software
 * wide surface before it is presented. The OpenGL compositor does the same
 * operation with wide_blit_center(). Software previously relied entirely on
 * per-primitive mirroring, which meant framebuffer content written through a
 * CPU->VRAM upload or GP0 VRAM copy (common on loading/menu screens) could be
 * correct in canonical VRAM but stale or black in the wide surface.
 *
 * Only the centre [offset, offset + native_w) is replaced. The synthetic
 * reveal margins remain owned by the native-wide mirror. At scale > 1 the
 * hi-res canonical mirror is authoritative for presentation, so splice from
 * g_hr; at scale 1 splice directly from guest-visible VRAM. X wraps exactly
 * like PSX VRAM scanout, which also covers a framebuffer based near x=1024. */
static void wide_splice_canonical_row(uint16_t *wide_row, int scaled_y,
                                      int base_x) {
    int s = g_scale;
    int native_w = g_wide_w - 2 * g_wide_off;
    int dst_x = g_wide_off * s;
    int copy_w = native_w * s;
    const uint16_t *canonical = g_hr ? g_hr : g_vram;
    int canonical_w = g_hr ? g_hr_w : VRAM_WIDTH;
    int canonical_h = g_hr ? g_hr_h : VRAM_HEIGHT;

    if (!wide_row || !canonical || native_w <= 0 || copy_w <= 0 ||
        canonical_w <= 0 || canonical_h <= 0)
        return;

    /* Defensive clipping keeps malformed experimental aspects from escaping
     * the sidecar allocation. Normal native-wide geometry has dst_x >= 0 and
     * dst_x + copy_w <= g_wide_w * scale. */
    if (dst_x < 0) {
        copy_w += dst_x;
        dst_x = 0;
    }
    if (dst_x + copy_w > g_wide_w * s)
        copy_w = g_wide_w * s - dst_x;
    if (copy_w <= 0)
        return;

    int src_y = scaled_y % canonical_h;
    if (src_y < 0) src_y += canonical_h;
    const uint16_t *src_row = canonical + (size_t)src_y * canonical_w;
    int src_x = (base_x * s) % canonical_w;
    if (src_x < 0) src_x += canonical_w;

    /* Usually one memcpy. Split only when the display band wraps at the VRAM
     * right edge. */
    int copied = 0;
    while (copied < copy_w) {
        int chunk = canonical_w - src_x;
        if (chunk > copy_w - copied) chunk = copy_w - copied;
        memcpy(wide_row + dst_x + copied, src_row + src_x,
...
void sw_wide_set_mirror_authoritative(int authoritative) {
    g_wide_mirror_authoritative = authoritative ? 1 : 0;
    if (!g_wide_mirror_authoritative)
        g_wide_sidecar_complete = 0;
}

void sw_wide_set_sidecar_complete(int complete) {
    g_wide_sidecar_complete = complete ? 1 : 0;
}
...
int sw_render_wide_display(uint32_t *out_pixels, int out_pitch, int base_x,
    ...
    int H = VRAM_HEIGHT * s;
    int out_h = disp_h * s;
    int splice_center = psx_ws_wide_center_splice_required(
        0, g_wide_mirror_authoritative, g_wide_sidecar_complete);
    int count = 0;
    for (int row = 0; row < out_h; row++) {
        int vy = disp_y * s + row;
        if (vy < 0) vy = 0;
        if (vy >= H) vy = H - 1;
        uint16_t *src = surf + (size_t)vy * W;
        if (splice_center)
            wide_splice_canonical_row(src, vy, base_x);
        uint32_t *dst = (uint32_t *)((uint8_t *)out_pixels + row * out_pitch);
...
int sw_wide_dump_full(uint32_t *out, int cap_pixels, int *ow, int *oh, int base_x) {
    ...
    int splice_center = psx_ws_wide_center_splice_required(
        0, g_wide_mirror_authoritative, g_wide_sidecar_complete);
    if ((long long)W * H > cap_pixels) {
        g_wide_mirror_authoritative = 0;
        return 0;
    }
    for (int row = 0; row < H; row++) {
        uint16_t *src = surf + (size_t)row * W;
        uint32_t *dst = out + (size_t)row * W;
        if (splice_center)
            wide_splice_canonical_row(src, row, base_x);
        for (int col = 0; col < W; col++)
            dst[col] = rgb555_to_argb(src[col]);
    }
    if (ow) *ow = W;
    if (oh) *oh = H;
    return W * H;
