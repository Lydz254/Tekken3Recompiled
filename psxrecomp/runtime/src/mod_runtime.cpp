...
void build_disc_index(RuntimeMods& s) {
    ...
    for (size_t i = 0; i < s.plan.overlays.size(); ++i) {
        ...
        for (size_t payload_offset = 0;
             payload_offset < overlay.payload.size();
             payload_offset += chunk_size) {
            const uint64_t chunk_index = payload_offset / chunk_size;
            const uint64_t location =
                overlay.location + chunk_index * overlay.stride;
            const uint64_t first = location / sector_size;
            const uint64_t last =
                (location + chunk_size - 1) / sector_size;
            for (uint64_t lba = first; lba <= last; ++lba) {
                auto& owners = index[(uint32_t)lba];
...
bool sha256_file(const std::filesystem::path& path, std::string& out,
    ...
    /* Keep this bounded well below Windows' default 1 MiB thread stack. The
     * old 1 MiB local array overflowed before this function's first statement
     * in standalone loader tests (and could do the same on small-stack host
     * threads). Streaming SHA-256 does not benefit from such a large chunk. */
...
bool sha256_disc_range(const std::filesystem::path& image,
                       ...
                       uint64_t chunk_size, uint64_t stride,
    ...
    if (extension == ".chd") {
        ...
        for (uint64_t payload_offset = 0; payload_offset < size;
            ...
            while (remaining != 0) {
                ...
                if (!read_ok) {
                    if (error)
                        *error = "cannot decode stock CHD overlay range";
                    return false;
                }
                const size_t chunk =
    ...
    for (uint64_t payload_offset = 0; payload_offset < size;
        ...
        while (remaining != 0) {
            ...
            if (physical > (uint64_t)file_size ||
                ...
                if (error)
                    *error = "overlay expected range exceeds stock image";
                return false;
            }
            file.clear();
...
bool mod_runtime_commit(const std::filesystem::path& disc_path, std::string* error) {
    ...
    for (const ModResolution::Overlay& overlay : plan.overlays) {
        ...
        if (!sha256_disc_range(
                ...
                overlay.payload.size(), overlay.chunk_size, overlay.stride,
                actual, &s.error) ||
    ...
    if (overlay_sector != overlay_index.end()) {
        for (size_t overlay_index_value : overlay_sector->second) {
            ...
            const size_t chunk_size =
                static_cast<size_t>(overlay.chunk_size);
            const uint64_t chunk_count =
                overlay.chunk_count;
            uint64_t first_chunk = 0;
            if (base > overlay.location &&
                base - overlay.location >= overlay.chunk_size) {
                first_chunk =
                    (base - overlay.location - overlay.chunk_size) /
                        overlay.stride +
                    1;
            ...
            for (uint64_t chunk_index = first_chunk;
                 chunk_index < last_chunk; ++chunk_index) {
                const size_t payload_offset =
                    static_cast<size_t>(chunk_index) * chunk_size;
                const uint64_t chunk_location =
                    overlay.location + chunk_index * overlay.stride;
                const uint64_t chunk_end = chunk_location + chunk_size;
                const uint64_t copy_begin = std::max(base, chunk_location);
                const uint64_t copy_end = std::min(end, chunk_end);
                if (copy_begin >= copy_end) continue;
                const size_t destination =
                    static_cast<size_t>(copy_begin - base);
                const size_t source = payload_offset +
                    static_cast<size_t>(copy_begin - chunk_location);
                const size_t count =
