...
/* Presentation gains are deliberately outside the emulated SPU snapshot.
 * Keep the canonical DSP, capture RAM and HQ verification oracle untouched. */
static int host_sfx_percent = 100, host_music_percent = 100;
static int16_t host_mix[2048 * 2], host_canonical[2048 * 2];
void spu_set_host_mix_volume(int sfx_percent, int music_percent) {
    host_sfx_percent = sfx_percent < 0 ? 0 : sfx_percent > 100 ? 100 : sfx_percent;
    host_music_percent = music_percent < 0 ? 0 : music_percent > 100 ? 100 : music_percent;
}
static int16_t host_mix_sample(int32_t total, int32_t music, int16_t main) {
    int32_t mixed = ((total - music) * host_sfx_percent + music * host_music_percent) / 100;
    return clamp16(((int32_t)clamp16(mixed) * main) >> 15);
}
static void host_finish_mix(int16_t *out, int frames, int custom) {
    if (custom) memcpy(host_canonical, out, (size_t)frames * 2 * sizeof(*out));
    spu_shadow_process(out, frames);
    if (custom) {
        /* The verified float enhancement models voices. Its accepted delta
         * belongs to SFX and must receive that gain too. */
        for (int i = 0; i < frames * 2; i++)
            out[i] = clamp16(host_mix[i] +
                ((int32_t)out[i] - host_canonical[i]) * host_sfx_percent / 100);
    }
}

void spu_render(int16_t* out_stereo, int frames) {
    if (!out_stereo || frames <= 0) return;
    int custom = host_sfx_percent != 100 || host_music_percent != 100;
    if (custom && frames > 2048) {
        for (int off = 0; off < frames; off += 2048) {
            int n = frames - off;
            spu_render(out_stereo + off * 2, n > 2048 ? 2048 : n);
        }
        return;
    }
    if (custom) memset(host_mix, 0, (size_t)frames * 2 * sizeof(*host_mix));
    ...
    if (enabled && !any_voice && !s_shadow_tap_on) {
        ...
        for (int f = 0; f < frames; f++) {
...

            {
                ...
                mix_l += wet_l;
                mix_r += wet_r;
                if (cd_rev && !eon) { music_l += wet_l; music_r += wet_r; }
            }

            if (custom) {
                host_mix[f * 2] = host_mix_sample(mix_l, music_l, main_l);
                host_mix[f * 2 + 1] = host_mix_sample(mix_r, music_r, main_r);
            }
            mix_l = clamp16(mix_l);
            mix_r = clamp16(mix_r);
        ...
        host_finish_mix(out_stereo, frames, custom);
