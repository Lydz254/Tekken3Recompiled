...
static HANDLE s_image_warm_thread = NULL;
static HANDLE s_image_warm_cancel_event = NULL;
...
static DWORD WINAPI overlay_image_warm_main(LPVOID unused) {
    ...
    if (now < s_image_warm_not_before_ms &&
        WaitForSingleObject(s_image_warm_cancel_event,
                            (DWORD)(s_image_warm_not_before_ms - now)) ==
            WAIT_OBJECT_0) {
        for (int i = 0; i < batch->count; i++) {
            int ci = batch->jobs[i].cache_idx;
    ...
    for (int i = 0; i < batch->count; i++) {
        ...
        if (WaitForSingleObject(s_image_warm_cancel_event, 0) == WAIT_OBJECT_0) {
            for (; i < batch->count; i++) {
                ci = batch->jobs[i].cache_idx;
                InterlockedCompareExchange(&s_image_warm[ci].state, 3, 1);
                InterlockedDecrement(&g_overlay_image_warm_pending);
            }
            break;
...
static void overlay_image_warm_init(void) {
    s_image_warm_enabled = 1;
    const char *e = getenv("PSX_OVERLAY_IMAGE_WARM");
    if (e && (e[0] == '0' || e[0] == 'n' || e[0] == 'N'))
        s_image_warm_enabled = 0;
    if (!s_image_warm_cancel_event)
        s_image_warm_cancel_event = CreateEventA(NULL, TRUE, FALSE, NULL);
    else
        ResetEvent(s_image_warm_cancel_event);
...
static void overlay_image_warm_queue(const int *indices, int count) {
    ...
    if (!s_image_warm_enabled || s_capacity_warm_dropped ||
        s_cand_n >= CAND_CAP || count <= 0 || !s_image_warm_cancel_event)
        return;
    /* Init normally queues once. If a future rescan wants to queue again,
     * reclaim a completed worker but never overlap two batches that mutate the
     * same per-cache-entry state array. */
    if (s_image_warm_thread) {
        if (WaitForSingleObject(s_image_warm_thread, 0) != WAIT_OBJECT_0)
            return;
        CloseHandle(s_image_warm_thread);
        s_image_warm_thread = NULL;
    }
    ...
    s_image_warm_thread =
        CreateThread(NULL, 0, overlay_image_warm_main, batch, 0, NULL);
    if (!s_image_warm_thread) {
        for (int i = 0; i < batch->count; i++) {
            InterlockedDecrement(&g_overlay_image_warm_pending);
    ...
    SetThreadPriority(s_image_warm_thread, THREAD_PRIORITY_BELOW_NORMAL);
}
...
static void overlay_image_warm_shutdown(void) {
    if (s_image_warm_cancel_event) SetEvent(s_image_warm_cancel_event);
    overlay_image_warm_drop_all();
    if (s_image_warm_thread) {
        /* A LoadLibrary already inside the Windows loader lock is allowed to
         * finish; the event prevents the worker from starting another image.
         * Joining makes every speculative HMODULE release race-free. */
        WaitForSingleObject(s_image_warm_thread, INFINITE);
        CloseHandle(s_image_warm_thread);
        s_image_warm_thread = NULL;
    }
    overlay_image_warm_drop_all();
    if (s_image_warm_cancel_event) {
        CloseHandle(s_image_warm_cancel_event);
        s_image_warm_cancel_event = NULL;
    }
    memset(s_image_warm, 0, sizeof(s_image_warm));
    InterlockedExchange(&g_overlay_image_warm_loaded, 0);
    InterlockedExchange(&g_overlay_image_warm_pending, 0);
}
#else
...
static void init_callbacks(void) {
     ...
     * native overlays charge cycles on the SAME timeline as the interp/BIOS. */
    {
        ...
        }
        {
            ...
            extern uint32_t psx_ws_full_angle_widen(uint32_t vanilla);
            s_callbacks.ws_full_angle_widen = psx_ws_full_angle_widen;
        }
    }
}
...
void overlay_loader_shutdown(void) {
    overlay_image_warm_shutdown();
}
 ...
 * call, too late to hide Windows' first image-map cost. A compact per-title
 * cache can be mapped in full without the multi-GB blow-up seen in mature
 * additive vaults: Tekken 3's release set is 49 DLLs / ~9 MiB, while the large
 * Tomba vault that motivated scoped warming has hundreds of artifacts. Bound
 * both file count and image bytes; larger inventories retain the historical
 * small-bundle/boot-text-base policy. Mapping never registers a candidate and
 * never bypasses manifest/live-RAM validation. */
#define IMAGE_WARM_SMALL_CACHE_MAX_DLLS 64
#define IMAGE_WARM_SMALL_CACHE_MAX_BYTES (64ull * 1024ull * 1024ull)
static void overlay_image_warm_seed_boot_text(void) {
    ...
    for (int ci = 0; ci < s_cache_idx_count; ci++) {
        CacheEntry *e = &s_cache_idx[ci];
        if (e->resident || e->capacity_suppressed || !e->manifest_ok ||
            e->func_count <= 0 || e->indexed_func_count != e->func_count)
            continue;
        WIN32_FILE_ATTRIBUTE_DATA attrs;
        if (!GetFileAttributesExA(e->path, GetFileExInfoStandard, &attrs)) {
            compact = 0;
            break;
        }
        uint64_t bytes = ((uint64_t)attrs.nFileSizeHigh << 32) |
                         (uint64_t)attrs.nFileSizeLow;
        if (count >= IMAGE_WARM_SMALL_CACHE_MAX_DLLS ||
            bytes > IMAGE_WARM_SMALL_CACHE_MAX_BYTES - total_bytes) {
            compact = 0;
            break;
        }
        indices[count++] = ci;
