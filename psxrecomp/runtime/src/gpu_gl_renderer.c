...
static GLint         s_present_uHoleMargin = -1;
...
static GLint         s_interp_uHoleMargin = -1;
...
static int           s_interp_hole_margin = 0;
...
static char s_skin_dir[HD_SKIN_COUNT][1024];
...
void gl_renderer_set_hd_texture_pack(const char *directory) {
    snprintf(s_hd_dir, sizeof s_hd_dir, "%s", directory ? directory : "");
}

void gl_renderer_set_skin_texture_pack(const char *directory) {
    snprintf(s_skin_dir[0], sizeof s_skin_dir[0], "%s", directory ? directory : "");
}

void gl_renderer_set_xiaoyu_texture_pack(const char *directory) {
    snprintf(s_skin_dir[1], sizeof s_skin_dir[1], "%s", directory ? directory : "");
}

void gl_renderer_set_anna_texture_pack(const char *directory) {
    snprintf(s_skin_dir[2], sizeof s_skin_dir[2], "%s", directory ? directory : "");
}

void gl_renderer_set_kuma_texture_pack(const char *directory) {
    snprintf(s_skin_dir[T3_SKIN_KUMA], sizeof s_skin_dir[T3_SKIN_KUMA], "%s", directory ? directory : "");
}
...
static GLuint hd_load_image(const char *directory, const char *name, int repeat_y) {
    ...
    if (pixels && fread(pixels, 1, size, f) == size && fgetc(f) == EOF) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, (int)w, (int)h, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels);
        /* Mipmaps suppress high-frequency ground shimmer at grazing angles. */
        typedef void (APIENTRY *HdGenerateMipmap)(GLenum);
        HdGenerateMipmap generate_mipmap = (HdGenerateMipmap)SDL_GL_GetProcAddress("glGenerateMipmap");
        if (generate_mipmap) {
...
static void hd_load_pack(void) {
    char path[1200]; const char *disabled = getenv("PSX_HD_TEXTURES");
    s_hd_tex[0] = s_hd_tex[1] = 0; s_hd_seen = 0;
    hd_map_clear(&s_hd_map);
    if (!s_hd_dir[0] || (disabled && !strcmp(disabled, "0"))) return;
    snprintf(path, sizeof path, "%s/mapping.bin", s_hd_dir);
    if (hd_map_load(&s_hd_map, path)) {
        s_hd_tex[0] = hd_load_image(s_hd_dir, "background", 0);
        s_hd_tex[1] = hd_load_image(s_hd_dir, "ground", 1);
        if (s_hd_tex[0] && s_hd_tex[1]) {
            fprintf(stdout, "HD textures: %u guarded forest tiles enabled\n", s_hd_map.count);
            return;
        }
    }
    fprintf(stdout, "HD textures: incomplete pack at %s; using original textures\n", s_hd_dir);
...
#endif
    snprintf(s_gallery_dir,sizeof s_gallery_dir,"%smods/outfit-menu",base?base:"");
#if !defined(PSX_SDL3)
    SDL_free(base);
#endif
    char catalog_path[1300];
    snprintf(catalog_path,sizeof catalog_path,"%s/catalog.txt",s_gallery_dir);
    (void)tekken3_outfits_load_catalog(catalog_path);
    for (int i = 0; i < HD_SKIN_COUNT; ++i) {
        char path[1200]; const char *disabled = getenv(s_skin_env[i]);
        s_skin_tex[i] = 0;
        tekken3_outfits_set_available(i, 0);
        hd_map_clear(&s_skin_map[i]);
        if (!s_skin_dir[i][0] || (disabled && !strcmp(disabled, "0"))) continue;
        snprintf(path, sizeof path, "%s/mapping.bin", s_skin_dir[i]);
        if (hd_map_load(&s_skin_map[i], path)) {
            /* A character map must only contain character-atlas records. */
            int valid = 1;
            for (uint32_t k = 0; k < s_skin_map[i].count; ++k)
                if (s_skin_map[i].tiles[k].kind != 3) valid = 0;
            if (valid) s_skin_tex[i] = hd_load_image(s_skin_dir[i], s_skin_name[i], 0);
            if (s_skin_tex[i]) {
                tekken3_outfits_set_available(i, 1);
                for (uint32_t k = 0; k < s_skin_map[i].count; ++k)
                    s_skin_map[i].tiles[k].kind = (uint32_t)(3 + i);
                fprintf(stdout, "%s skin: %u guarded costume tiles enabled\n", s_skin_label[i], s_skin_map[i].count);
                continue;
            }
        }
        fprintf(stdout, "%s skin: incomplete pack at %s; using original costume\n", s_skin_label[i], s_skin_dir[i]);
        hd_map_clear(&s_skin_map[i]);
    }
}
...
/* Armed only when this frame actually submits an exact nw_backdrop band. This
 * prevents the Tekken opt-in from edge-filling unrelated menus/loading frames. */
static unsigned char s_wide_hole_fill[WIDE_MAX_SURF];
...
static int    s_wide_prim_suppressed = 0;
...
static inline int wide_dx(void) {
    return g_wide_off - g_wide_cur_base + s_wide_prim_dx;
}
...
static int    s_hold_hole_margin = 0;
...
static void hold_capture_native_fbo(GLuint src_fbo, int dx, int dy, int dw, int dh,
                                    int force_4_3, int linear, int hole_margin) {
    ...
    s_hold_hole_margin = hole_margin;
}
...
static const char *PRESENT_FS =
    "#version 330\n"
    "in vec2 v_uv; uniform sampler2D u_tex; uniform int u_hole_margin; out vec4 frag;\n"
    "vec4 ws_sample(sampler2D t, vec2 uv){\n"
    "  ivec2 sz=textureSize(t,0);\n"
    "  ivec2 p=clamp(ivec2(uv*vec2(sz)),ivec2(0),sz-ivec2(1));\n"
    "  if(u_hole_margin>0 && (p.x<u_hole_margin || p.x>=sz.x-u_hole_margin)){\n"
    "    vec4 c=texelFetch(t,p,0);\n"
    "    if(c.a>0.49 && c.a<0.51){\n"
    "      p.x=(p.x<u_hole_margin)?u_hole_margin:sz.x-u_hole_margin-1;\n"
    ...
    "  return texture(t,uv);\n"
    "}\n"
    "void main(){ frag=ws_sample(u_tex,v_uv); }\n";
static const char *INTERP_FS =
    "#version 330\n"
    "in vec2 v_uv; uniform sampler2D u_prev; uniform sampler2D u_curr;\n"
    "uniform float u_alpha; uniform int u_blend_mode; uniform int u_hole_margin; out vec4 frag;\n"
    "vec4 ws_sample(sampler2D t, vec2 uv){\n"
    "  ivec2 sz=textureSize(t,0);\n"
    "  ivec2 p=clamp(ivec2(uv*vec2(sz)),ivec2(0),sz-ivec2(1));\n"
    "  if(u_hole_margin>0 && (p.x<u_hole_margin || p.x>=sz.x-u_hole_margin)){\n"
    ...
    "}\n"
    "void main(){\n"
    "  vec4 prev=ws_sample(u_prev,v_uv), curr=ws_sample(u_curr,v_uv);\n"
    "  float alpha=u_alpha;\n"
    "  if(u_blend_mode==1){\n"
...
static const char *TEX_VS =
    ...
    "layout(location=10) in vec2 a_hd_uv; layout(location=11) in float a_hd_kind;\n"
    ...
    "  v_hd_uv = a_hd_uv; v_hd_uv_p = a_hd_uv; v_hd_kind = int(a_hd_kind + 0.5);\n"
    "  v_persp = (a_q > 0.0) ? 1 : 0;\n"
...
static const char *TEX_FS =
    ...
    "    if (v_hd_kind == 1) rgb = texture(u_hd_background, huv).rgb;\n"
    "    else if (v_hd_kind == 2) rgb = texture(u_hd_ground, huv).rgb;\n"
...
static void wide_target_begin_fbo(GLuint fbo, int dx, GLint uXoff, GLint uXhalf) {
    ...
    glEnable(GL_SCISSOR_TEST);
    {
        int sy = s_wide_prim_unclipped ? 0 : s_area_y1;
        int sh = s_wide_prim_unclipped ? VRAM_H
...
extern int gpu_ws_nw_background_history_enabled(void);
...
static int bd_prim_gate(const int *xs, int n, int textured) {
    ...
    /* Exact full-frame `nw_backdrop` commands and legacy tagged backdrops are
     * both resolved in gpu.c while the complete GP0 command/provenance is live. */
    {
        int backdrop_mode = psx_ws_prim_in_backdrop();
        if (backdrop_mode) return backdrop_mode;
    }
    if (g_ws_bd_phase_mode != 0) return 0;
...
/* Independent of raster policy: complete-mirror gameplay may own the centre,
 * while quiet/upload-heavy 2D frames still request the canonical splice. */
static int s_wide_mirror_authoritative = 0;
static int s_wide_sidecar_complete = 0;
...
static int mirror_x_center_only(int lo, int hi) {
    /* An exact layer-aware 2D layout, like Tekken 3's selector, owns its
     * complete sidecar for this frame. It must rasterize centre content too;
     * presentation deliberately skips the canonical centre splice. */
    if (!s_wide_fast || s_wide_sidecar_complete) return 0;
...
static void wide_set_bd_scale(GLint uScale, GLint uCenter) {
    ...
    if (s_bd_gate && (g_ws_bd_stretch_on || s_bd_gate == 3) && g_wide_w > 0) {
        int native_w = g_wide_w - 2 * g_wide_off;
        if (native_w > 0) {
            scale  = s_bd_gate == 3 ? (float)g_wide_w / (float)native_w
                                    : (g_ws_bd_stretch_pct > 0
                                       ? (float)g_ws_bd_stretch_pct / 100.0f
...
static void flush_tex_batch(void) {
    ...
    glBindTexture(GL_TEXTURE_2D, s_hd_tex[0]);
    p_glUniform1i(s_uHdBackground, 1);
    p_glActiveTexture(PSXGL_TEXTURE0 + 2);
    glBindTexture(GL_TEXTURE_2D, s_hd_tex[1]);
    p_glUniform1i(s_uHdGround, 2);
    for (int i = 0; i < HD_SKIN_COUNT; ++i) {
        p_glActiveTexture(PSXGL_TEXTURE0 + 3 + i);
...
static void gpu_textured_triangle(const int *xs, const int *ys,
    ...
    int original_us[3], original_vs[3];
    for (int i = 0; i < 3; ++i) { original_us[i] = us[i]; original_vs[i] = vs[i]; }
    int lim_buf[4];
    int uv_buf[6];
    if (!lim) {
     ...
     * state goes in the vertex; only these keys force a new draw. */
    {
        ...
        int gate = s_wide_prim_expanded ? 3 : bd_prim_gate(xs, 3, 1);
        ...
        for (int i = 0; i < 3; i++, vp += TEXV) {
            ...
            float hu = (float)(base_x * (depth == 1 ? 2 : 4) + original_us[i]);
            float hv = (float)(base_y + original_vs[i]);
            vp[20] = hd ? hd->map[0]*hu + hd->map[1]*hv + hd->map[2] : 0.0f;
            vp[21] = hd ? hd->map[3]*hu + hd->map[4]*hv + hd->map[5] : 0.0f;
            vp[22] = hd ? (float)hd->kind : 0.0f;
        }
        s_tb_n += 3;
...
static void gpu_flat_rect(int x,int y,int w,int h,uint16_t c,int semi) {
    ...
    if (overlay) {
         ...
         * clean. */
        if (!s_wide_prim_suppressed && s_ws_ablate != 1) {
            hr_begin(0);
            gl_perf_mirror_begin();
...
static int init_gpu_raster(void) {
    ...
    p_glBindBuffer(PSXGL_ARRAY_BUFFER, s_tex_vbo);
    {
        ...
        p_glVertexAttribPointer(10, 2, GL_FLOAT, GL_FALSE, st, (void*)(20*sizeof(float))); p_glEnableVertexAttribArray(10);
        p_glVertexAttribPointer(11, 1, GL_FLOAT, GL_FALSE, st, (void*)(22*sizeof(float))); p_glEnableVertexAttribArray(11);
    }
...
int gl_renderer_init_context(SDL_Window *win) {
    ...
    if (ok) {
        ...
        if (s_present_prog && s_interp_prog) {
            ...
            s_present_uHoleMargin =
                p_glGetUniformLocation(s_present_prog, "u_hole_margin");
            ...
            s_interp_uHoleMargin =
                p_glGetUniformLocation(s_interp_prog, "u_hole_margin");
...
void gl_renderer_present(const uint32_t *pixels, int src_w, int src_h, int linear,
    ...
    p_glUniform1i(s_present_uHoleMargin, 0);
    if (crop) {
...
static void glb_wide_disable_target(void) {
    flush_tex_batch();
    g_wide_cur = 0;
    s_wide_prim_dx = 0;
...
static void glb_wide_set_primitive_suppressed(int suppressed) {
    ...
    if (suppressed == s_wide_prim_suppressed) return;
    flush_flat_batch();
    flush_tex_batch();
    s_wide_prim_suppressed = suppressed;
}

static void glb_wide_set_primitive_unclipped(int unclipped) {
    unclipped = unclipped ? 1 : 0;
    if (unclipped == s_wide_prim_unclipped) return;
    flush_flat_batch();
    flush_tex_batch();
    s_wide_prim_unclipped = unclipped;
}

static void glb_wide_set_primitive_expanded(int expanded) {
    expanded = expanded ? 1 : 0;
    if (expanded == s_wide_prim_expanded) return;
    flush_flat_batch();
    flush_tex_batch();
    s_wide_prim_expanded = expanded;
}

static void glb_wide_set_mirror_authoritative(int authoritative) {
    s_wide_mirror_authoritative = authoritative ? 1 : 0;
    if (!s_wide_mirror_authoritative)
        s_wide_sidecar_complete = 0;
}

static void glb_wide_set_sidecar_complete(int complete) {
    s_wide_sidecar_complete = complete ? 1 : 0;
}
...
static void glb_wide_clear(int base_x, int y, int h, uint16_t color) {
    ...
    glStencilMask(0xFF);
    glClear(GL_COLOR_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);
    /* The guest clear owns the canonical framebuffer. Its full-wide mirror is
     * a convenience, not real margin coverage: put the host-only sentinel back
     * into those strips so following world draws punch exact coverage into it. */
    if (gpu_ws_nw_backdrop_enabled() && g_wide_off > 0) {
        int margin = g_wide_off * s_scale;
        glClearColor(r, g, b, (float)PSX_WS_HOLE_ALPHA / 255.0f);
        glClearStencil(0);
        glScissor(0, y0, margin, y1 - y0);
        glClear(GL_COLOR_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);
        glScissor(g_wide_w * s_scale - margin, y0, margin, y1 - y0);
        glClear(GL_COLOR_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);
    }
    glDisable(GL_SCISSOR_TEST);
    p_glBindFramebuffer(PSXGL_FRAMEBUFFER, 0);
...
static void glb_wide_clear_margins(int base_x, int y, int h, uint16_t color, int sides) {
    ...
    float g = ((color >> 5) & 0x1F) / 31.0f;
    float b = ((color >> 10) & 0x1F) / 31.0f;
    /* Alpha 0.5 cannot be produced by PS1 colour writes (mask alpha is 0/1).
     * For the opted-in backdrop reconstruction it marks only host-owned,
     * untouched margin texels; stencil remains the guest mask bit. */
    float a = gpu_ws_nw_backdrop_enabled()
        ? ((float)PSX_WS_HOLE_ALPHA / 255.0f)
        : ((color >> 15) & 1 ? 1.0f : 0.0f);
    p_glBindFramebuffer(PSXGL_FRAMEBUFFER, fbo);
    glViewport(0, 0, W, H);
    glEnable(GL_SCISSOR_TEST);
    glClearColor(r, g, b, a);
...
static int glb_render_wide_display(uint32_t *out, int pitch, int base_x,
    ...
    if (!s_raster_ok || !s_ctx || g_wide_w <= 0) {
        s_wide_mirror_authoritative = 0;
        return 0;
    }
    GLuint fbo = 0;
    int hole_fill = 0;
    for (int i = 0; i < WIDE_MAX_SURF; i++)
        if (s_wide_fbo[i] && s_wide_base[i] == base_x) {
            fbo = s_wide_fbo[i]; hole_fill = s_wide_hole_fill[i]; break;
        }
    if (!fbo) {
        s_wide_mirror_authoritative = 0;
        return 0;
    ...
    if (hole_fill)
        psx_ws_resolve_margin_holes_argb(
            tmp, W, W, out_h, g_wide_off * s_scale);
...
static int glb_wide_dump_full(uint32_t *out, int cap_pixels, int *ow, int *oh,
    ...
    int W = g_wide_w * s_scale;
    int H = VRAM_H * s_scale;
    if (cap_pixels > 0 && (long)W * H > cap_pixels) {
        H = cap_pixels / W;
        if (H <= 0) {
            s_wide_mirror_authoritative = 0;
            return 0;
...
static int interp_capture(GLuint fbo, int x, int y, int w, int h,
    ...
    s_interp_hole_margin = hole_margin;
    s_interp_captures++;
...
static void interp_draw_quad(float alpha, int lx, int ly, int lw, int lh) {
    ...
    p_glUniform1i(s_interp_uHoleMargin, s_interp_hole_margin);
...
static void present_target_quad(GLuint tex, int tex_w, int tex_h,
    ...
    p_glUseProgram(s_present_prog);
    p_glUniform1i(s_present_uTex, 0);
    p_glUniform1i(s_present_uHoleMargin, hole_margin);
...
int gl_renderer_present_hold_last(void) {
    ...
    if (s_hold_kind == HOLD_DRAWABLE) {
        ...
        present_target_quad(s_hold_tex, s_hold_tw, s_hold_th,
                            ...
                            lx, ly, lw, lh, 1, s_hold_hole_margin);
...
void gl_renderer_present_vram(int disp_x, int disp_y, int w, int h, int linear,
    ...
    int interp_pair = interp_capture(s_hr_fbo, disp_x, disp_y, w, h,
                                     linear, force_4_3, GL_PRES_VRAM, 0);
    if (interp_pair) {
    ...
    present_target_quad(s_hr_tex, VRAM_W, VRAM_H,
                        disp_x, disp_y, w, h, linear, lx, ly, lw, lh, 1, 0);
 ...
 * x-translated by the reveal offset. Full-mirror gameplay can make the wide
 * surface authoritative and bypass this copy; quiet/upload-heavy 2D retains
 * it. Shared by direct present, CPU readback, and full-surface dump. */
static void wide_blit_center(GLuint wide_fbo, int base_x, int disp_y, int disp_h) {
    if (!psx_ws_wide_center_splice_required(
            s_wide_fast, s_wide_mirror_authoritative,
            s_wide_sidecar_complete) ||
        g_wide_w <= 0)
        return;
    int native_w = g_wide_w - 2 * g_wide_off;
...
int gl_renderer_present_wide_fbo(int disp_x, int disp_y, int disp_h, int linear) {
    if (!s_ctx || !s_raster_ok || g_wide_w <= 0) {
        s_wide_mirror_authoritative = 0;
        return 0;
    }
    GLuint fbo = 0, tex = 0;
    ...
    for (int i = 0; i < WIDE_MAX_SURF; i++)
        if (s_wide_fbo[i] && s_wide_base[i] == disp_x) {
            ...
            hole_fill = s_wide_hole_fill[i]; break;
        }
    if (!fbo) {
        s_wide_mirror_authoritative = 0;
        return 0;
    ...
    int interp_pair = interp_capture(fbo, 0, disp_y, g_wide_w, disp_h,
                                     ...
                                     hole_fill ? g_wide_off * s_scale : 0);
    if (interp_pair) {
...
static const GpuRenderBackend GL_BACKEND = {
    ...
    .wide_set_mirror_authoritative = glb_wide_set_mirror_authoritative,
    .wide_set_sidecar_complete = glb_wide_set_sidecar_complete,
