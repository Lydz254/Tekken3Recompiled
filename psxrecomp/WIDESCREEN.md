...
nw_2d_edge_fill    = true              # on sustained GTE-quiet native-wide
                                       # frames, fill only the synthetic side
                                       # margins from an averaged edge palette;
                                       # centre UI/art remains pixel-exact.
...
nw_hud_packet2_lo = "0x000F3400"      # optional second half-open HUD range for
nw_hud_packet2_hi = "0x000F4100"      # double-buffered/discontiguous arenas.
...
`nw_2d_edge_fill` is also runtime-only. It is intended for a mixed title whose
native-wide gameplay renders genuine side geometry, while title, menu, loading,
or selection screens contain a centred 4:3 composition. After three consecutive
frames without a GTE projection, the present path replaces only the synthetic
left/right columns with a bounded horizontal and vertical average of each
adjacent edge, eased slightly darker toward the outside. It never rescales the
canonical image and never reflects individual edge pixels, so readable UI is
not duplicated into the margins. Any GTE projection immediately returns the
margins to renderer-owned world geometry. Depth24 and recently active MDEC
frames remain authored 4:3 and never enter the fill path. OpenGL and software
use the same ARGB compositor for these 2D frames; OpenGL gameplay retains its
GPU-direct native-wide present. The option defaults off and 4:3 is an exact
identity.
