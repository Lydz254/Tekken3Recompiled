...
def read_iso_file(read_user, data: bytes, extent: int, size: int) -> bytes:
    out = bytearray()
    rem, lba = size, extent
    while rem > 0:
...
def collect_iso_files(
    ...
    directory = read_iso_file(read_user, data, extent, size)
...
def extract_via(
    ...
    for need in ("SYSTEM.CNF", boot_exe):
        try:
            disc_path = resolve_iso_file(entries, need)
        except KeyError:
            raise SystemExit(f"missing {need} on disc (found {sorted(entries)[:20]})")
        extent, size = entries[disc_path]
        files[need] = read_iso_file(read_user, data, extent, size)
