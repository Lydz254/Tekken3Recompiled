...
def complete_delay_slot_boundary_captures(captures: list) -> tuple:
    """Complete a truncated final delay slot from agreeing captured bytes.

    A dirty-region upload can end on a control-transfer word while the mandatory
    delay slot lives in an adjacent/coalesced capture.  Zero padding is unsafe:
    the delay slot is executable state.  Instead, extend the recipe by exactly
    one word only when another retained capture covers the whole source interval
    plus that word, its overlap is byte-identical, and every such provider agrees
    on the missing word.  Ambiguous or unproven boundaries keep their existing
    loud recompiler failure.

    Returns ``(completed_captures, completion_events)`` without mutating the
    retained capture vault.
    """
    completed = [dict(cap) for cap in captures]
    decoded = []
    for cap in captures:
        try:
            data = base64.b64decode(cap['bytes_b64'])
            size = int(cap['size'])
            raw_load = cap['load_addr']
            load_addr = (int(raw_load, 0) if isinstance(raw_load, str)
                         else int(raw_load))
        except (KeyError, TypeError, ValueError, binascii.Error):
            decoded.append(None)
            continue
        if size != len(data) or size < 4 or (size & 3):
            decoded.append(None)
            continue
        decoded.append((load_addr & 0x1FFFFFFF, data))

    events = []
    for source_i, source_item in enumerate(decoded):
        if source_item is None:
            continue
        source_base, source_data = source_item
        tail = int.from_bytes(source_data[-4:], 'little')
        if _mips_control_kind(tail) != 1:
            continue
        source_end = source_base + len(source_data)

        provider_words = []
        for provider_i, provider_item in enumerate(decoded):
            if provider_i == source_i or provider_item is None:
                continue
            provider_base, provider_data = provider_item
            provider_end = provider_base + len(provider_data)
            if (provider_base > source_base or
                    provider_end < source_end + 4):
                continue
            overlap_off = source_base - provider_base
            if provider_data[overlap_off:overlap_off + len(source_data)] != \
                    source_data:
                continue
            slot_off = source_end - provider_base
            provider_words.append(provider_data[slot_off:slot_off + 4])

        distinct_words = set(provider_words)
        if len(distinct_words) != 1:
            continue
        delay_word = distinct_words.pop()
        completed_data = source_data + delay_word
        completed[source_i]['size'] = len(completed_data)
        completed[source_i]['bytes_b64'] = base64.b64encode(
            completed_data).decode()
        events.append({
            'load_addr': source_base | 0x80000000,
            'original_size': len(source_data),
            'provider_count': len(provider_words),
            'delay_word': int.from_bytes(delay_word, 'little'),
        })
...
def main():
    ...
    captures, boundary_completions = complete_delay_slot_boundary_captures(
        captures)
    for completion in boundary_completions:
        print(f"Boundary capture 0x{completion['load_addr']:08X} "
              f"size={completion['original_size']} ended on a delayed transfer; "
              f"completed word=0x{completion['delay_word']:08X} from "
              f"{completion['provider_count']} exact-overlap provider(s)")
    if boundary_completions:
        print(f'Delay-slot boundary normalization: completed '
              f'{len(boundary_completions)} capture(s)\n')
