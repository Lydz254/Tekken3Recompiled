...
Package format 6 adds a bounded strided layout for interleaved disc media. It
keeps one authored payload file while claiming only the bytes that belong to
that stream:
...
The payload is exactly `chunk_size * chunk_count` bytes. Consecutive payload
chunks map to `offset + index * stride`; `stride` must be at least
`chunk_size`, and the count is capped at 65536. All three layout fields are
required together. A strided overlay's `expected_sha256` hashes the
concatenated stock chunks in payload order, excluding the gaps. Collision
claims likewise cover only those chunks. This is useful for eight-way XA
interleave without copying or replacing sibling channels. It is not a codec,
sector encoder, or permission to bundle stock data.
