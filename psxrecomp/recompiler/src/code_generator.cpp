    ...
    if (opcode == 0x01) {
        ...
        if ((regimm_op & 0x01u) == 0x00u) { // bltz family (incl. bltzal + undefined mirrors)
            ...
            if (regimm_op == 0x00 &&
                ...
                return config_.ws_cull_bltz_sites.count(addr)
                    ? fmt::format("psx_ws_background_cull_bltz({}) /* resident background left edge */",
                                  reg_name(rs))
                    : fmt::format("psx_ws_cull_bltz({}) /* ws cull (left edge) */",
                                  reg_name(rs));
            return fmt::format("(int32_t){} < 0", reg_name(rs));
        } else {                            // bgez family (incl. bgezal + undefined mirrors)
            if (regimm_op == 0x01 && config_.ws_cull_bltz_sites.count(addr))
                return fmt::format("!psx_ws_background_cull_bltz({}) /* resident background left keep */",
                                   reg_name(rs));
            return fmt::format("(int32_t){} >= 0", reg_name(rs));
        }
    }
...
std::string CodeGenerator::translate_instruction(uint32_t addr, uint32_t instr) {
    ...
    if (config_.ws_shell_index_site != 0u &&
        PSXRecompV4::recompiler_patch_address_key(
            config_.ws_shell_index_site) ==
        PSXRecompV4::recompiler_patch_address_key(addr)) {
        if (instr != config_.ws_shell_index_expected) {
            if (!config_.overlay_mode)
                throw std::runtime_error(fmt::format(
                    "[widescreen.shell] index site 0x{:08X} expected "
                    "0x{:08X}, found 0x{:08X}", addr,
                    config_.ws_shell_index_expected, instr));
        } else if (opcode == 0x2Bu) { // sw rt,off(rs)
            ...
            return fmt::format(
                ...
                "psx_ws_shell_source_index({}, {}u, {}u));"
                "  /* valid source descriptor for widened shell */{}",
                addr, reg_name(rs), (int)offset, reg_name(rt),
                config_.ws_shell_base_cols, config_.ws_shell_wide_cols,
                comment);
        } else if (!config_.overlay_mode) {
            throw std::runtime_error(fmt::format(
                "[widescreen.shell] index site 0x{:08X} is not SW "
                "(0x{:08X})", addr, instr));
        }
    }

    // Compile package-style instruction alternatives into AOT code instead of
    // writing code bytes at runtime. A main-EXE write marks its whole guest
    // text page executable-dirty; a hot render loop on that page would then run
    // through dirty_ram_interp. These deliberately narrow shapes cover finite
    // mesh coordinate/count extensions while retaining the stock instruction
    // whenever the widescreen reveal is inactive.
    for (const auto& patch : config_.ws_native_patches) {
        if (PSXRecompV4::recompiler_patch_address_key(patch.address) !=
            PSXRecompV4::recompiler_patch_address_key(addr)) continue;
        if (instr != patch.expected) {
            if (config_.overlay_mode) continue;
            throw std::runtime_error(fmt::format(
                "[widescreen.native_patch] '{}' expected 0x{:08X} at "
                "0x{:08X}, found 0x{:08X}", patch.id, patch.expected,
                addr, instr));
        }

        const uint32_t replacement = patch.replacement;
        const uint32_t replacement_opcode = (replacement >> 26) & 0x3Fu;
        const uint32_t replacement_funct = replacement & 0x3Fu;
        const bool same_regs =
            get_rs(instr) == get_rs(replacement) &&
            get_rt(instr) == get_rt(replacement) &&
            get_rd(instr) == get_rd(replacement);

        // SLL rd,rt,old_sa -> SLL rd,rt,new_sa.
        if (opcode == 0u && funct == 0u && replacement_opcode == 0u &&
            replacement_funct == 0u && same_regs) {
            const uint32_t old_sa = (instr >> 6) & 0x1Fu;
            const uint32_t new_sa = (replacement >> 6) & 0x1Fu;
            return fmt::format(
                "{} = {} << (psx_ws_x_margin() > 0 ? {}u : {}u);"
                "  /* native widescreen patch: {} */{}",
                reg_name(get_rd(instr)), reg_name(get_rt(instr)), new_sa,
                old_sa, patch.id, comment);
        }

        // ADDU rd,rs,rt -> SUBU rd,rs,rt (or the inverse).
        if (opcode == 0u && replacement_opcode == 0u && same_regs &&
            ((funct == 0x21u && replacement_funct == 0x23u) ||
             (funct == 0x23u && replacement_funct == 0x21u))) {
            const char* vanilla_op = funct == 0x21u ? "+" : "-";
            const char* wide_op = replacement_funct == 0x21u ? "+" : "-";
            return fmt::format(
                "{} = psx_ws_x_margin() > 0 ? ({} {} {}) : ({} {} {});"
                "  /* native widescreen patch: {} */{}",
                reg_name(get_rd(instr)), reg_name(get_rs(instr)), wide_op,
                reg_name(get_rt(instr)), reg_name(get_rs(instr)), vanilla_op,
                reg_name(get_rt(instr)), patch.id, comment);
        }

        // SLTI rt,rs,old_imm -> SLTI rt,rs,new_imm.
        if (opcode == 0x0Au && replacement_opcode == 0x0Au &&
            get_rs(instr) == get_rs(replacement) &&
            get_rt(instr) == get_rt(replacement)) {
            const int32_t old_imm = (int16_t)(instr & 0xFFFFu);
            const int32_t new_imm = (int16_t)(replacement & 0xFFFFu);
            return fmt::format(
                "{} = ((int32_t){} < (psx_ws_x_margin() > 0 ? {} : {})) "
                "? 1u : 0u;  /* native widescreen patch: {} */{}",
                reg_name(get_rt(instr)), reg_name(get_rs(instr)), new_imm,
                old_imm, patch.id, comment);
        }

        throw std::runtime_error(fmt::format(
            "[widescreen.native_patch] '{}' at 0x{:08X} uses unsupported "
            "instruction shape 0x{:08X} -> 0x{:08X}", patch.id, addr,
            instr, replacement));
    }
    ...
    for (const auto& site : config_.ws_cull_angle_sites) {
        ...
        return fmt::format(
            "{} = {}({}u);"
            "  /* aspect-scaled terrain visibility angle */{}",
            reg_name(dst), site.full_angle ? "psx_ws_full_angle_widen" : "psx_ws_angle_widen",
            (uint32_t)vanilla, comment);
    }
    ...
    if (config_.ws_cull_right_sub_sites.count(addr)) {
        if ((opcode == 0x08 || opcode == 0x09) && get_rs(instr) == get_rt(instr)) {
            uint32_t reg = get_rt(instr);
            int16_t imm = get_imm16(instr);
            if (imm >= 0 && !config_.overlay_mode) {
                fmt::print(stderr, "ERROR: [widescreen.cull] right_sub site 0x{:08X} "
                           "does not have a negative immediate (0x{:08X})\n", addr, instr);
                std::exit(1);
            }
            return fmt::format("{} = {} + ((int32_t){} - psx_ws_background_cull_margin());"
                               "  /* resident background right edge */{}",
                               reg_name(reg), reg_name(reg), imm, comment);
        } else if (!config_.overlay_mode) {
            fmt::print(stderr, "ERROR: [widescreen.cull] right_sub site 0x{:08X} "
                       "is not addi/addiu r,r,-imm (0x{:08X})\n", addr, instr);
            std::exit(1);
        }
    }
    if (config_.ws_cull_screen_x_sites.count(addr)) {
        const uint32_t expected = config_.ws_cull_screen_x_expected;
        if (expected != 0 && instr != expected) {
            if (!config_.overlay_mode) {
                throw std::runtime_error(fmt::format(
                    "[widescreen.cull] screen_x site 0x{:08X} expected "
                    "instruction 0x{:08X}, found 0x{:08X} "
                    "(wrong game revision or stale guard)",
                    addr, expected, instr));
            }
            // Overlay variants may map unrelated instructions at the same VA.
            // Do not apply the explicit transform unless the complete word
            // matches; continue to the normal translation below.
        } else if (opcode == 0x0B) {  // sltiu rt,rs,imm
            uint32_t rs = get_rs(instr), rt = get_rt(instr);
            uint16_t imm = get_imm16_u(instr);
    ...
    // Exact background-packet provenance ([widescreen.backdrop]
    // packet_tag_sites). These are the final `sw` writes made after a packet's
    // coordinates/UVs are complete. Record the packet address without changing
    // guest RAM; the wide renderer later expands only the tagged packet's outer
    // vertices. Normal stage geometry, fighters, and UI never enter this table.
    if (config_.ws_backdrop_packet_tag_sites.count(addr)) {
        if (opcode == 0x2B) {  // sw
            uint32_t rs = get_rs(instr), rt = get_rt(instr);
            int16_t offset = get_imm16(instr);
            std::string dst = offset == 0
                ? reg_name(rs)
                : fmt::format("{} + {}", reg_name(rs), offset);
            return fmt::format(
                "g_debug_last_store_pc = 0x{:08X}u; psx_store_cycle_barrier(); "
                "cpu->write_word({}, {}); psx_ws_background_packet_tag({});{}",
                addr, dst, reg_name(rt), dst, comment);
        } else if (!config_.overlay_mode) {
            fmt::print(stderr, "ERROR: [widescreen.backdrop] packet_tag site "
                       "0x{:08X} is not sw (opcode 0x{:02X})\n", addr, opcode);
            std::exit(1);
        }
    }
...
void CodeGenerator::emit_runtime_externs(std::ostream& ss) const {
    ...
    ss << "extern uint32_t psx_ws_shell_source_index(uint32_t index, uint32_t base_cols, uint32_t wide_cols); /* finite-shell descriptor map */\n";
    ...
    ss << "extern int  psx_ws_background_cull_bltz(uint32_t v);       /* resident background left edge */\n";
    ss << "extern int  psx_ws_background_cull_margin(void);           /* background-only overdraw margin */\n";
    ...
    ss << "extern uint32_t psx_ws_full_angle_widen(uint32_t vanilla); /* full aperture with live viewport width */\n";
    ss << "extern int  psx_ws_backdrop_x(int x);  /* widescreen backdrop screenX squash (gpu.c) */\n";
    ss << "extern void psx_ws_background_packet_tag(uint32_t addr); /* exact background packet provenance */\n";
