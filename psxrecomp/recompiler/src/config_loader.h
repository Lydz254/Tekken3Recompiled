...
struct WidescreenAngleSite {
    uint32_t address = 0;
    uint32_t expected = 0;
    bool full_angle = false; // Full aperture, halved by the guest visibility routine. // guarded ADDI/ADDIU with rs == zero
};
...
struct GameConfig {
    ...
    // addiu r,r,-W right-edge checks widened to -(W+x_margin).
    std::vector<uint32_t> ws_cull_right_sub_sites;
    ...
    // Optional complete-word guard shared by screen_x_sites. A nonzero value
    // makes regeneration fail if any main-EXE site no longer contains this
    // exact SLTIU instruction (for example, after switching game revisions).
    // Zero preserves the legacy opcode-only validation.
    ...
    // Additional margin used only by explicitly listed background polygon
    // bltz/right_sub sites. This lets a PC build keep a finite sky/stage mesh
    // resident without widening foreground render funnels or actor queues.
    ...
    // [widescreen.backdrop] packet_tag_sites: exact `sw` instructions that
    // finalize a background polygon packet. The recompiler records the packet
    // address after the store so only those packets can receive edge-only
    // expansion in the native-wide sidecar. Empty by default; regen required.
    ...
    // Uniform native-wide GTE projection scale. A ratio below one moves the
    // virtual camera back without changing guest simulation or collision.
    ...
    // [widescreen] nw_hud_packet2_lo / nw_hud_packet2_hi — optional second
    // half-open physical-RAM source range for a double-buffered or otherwise
    // discontiguous HUD packet pool. The legacy nw_left_hud_packet pair remains
    // range 1. Both values must be present together. Runtime-only; no regen.
    ...
    // [widescreen] nw_2d_edge_fill: on sustained GTE-quiet native-wide
    // frames, fill only the synthetic reveal margins from a blurred/averaged
    // edge palette. The canonical image is centred and bit-exact: no stretch,
    // no mirrored text. Any GTE activity and all MDEC/depth24 frames disable
    // it. Runtime-only; intended for full_2d mixed 2D/3D titles. Off by default.
    ...
    // [widescreen] nw_ui_stretch_min_ot — stretch final-layer rectangle/
    // sprite UI packets in the native-wide sidecar. Zero disables it.
    ...
    // [[widescreen.native_patch]]: exact instruction alternatives compiled
    // into the static native body. The expected word remains the 4:3 path and
    // the replacement is selected only while widescreen reveal is active.
    // This avoids runtime main-EXE writes, which dirty an entire code page and
    // force hot render functions through the executable-RAM interpreter.
    std::vector<RecompilerPatch> ws_native_patches;

    // [widescreen.shell] maps a widened row-major output index back into the
    // original finite descriptor grid. The added border gets valid wrapped
    // source records instead of reading whatever data follows the stock table.
