...
uint32_t overlay_codegen_config_hash(const GameConfig& c) {
    ...
    h.words("backdrop_packet_tag", c.ws_backdrop_packet_tag_sites);
    ...
    h.u32((uint32_t)c.ws_background_guard_pixels);
    ...
    std::vector<RecompilerPatch> ws_native_patches = c.ws_native_patches;
    std::sort(ws_native_patches.begin(), ws_native_patches.end(),
              [](const auto& a, const auto& b) {
                  const uint32_t aa = recompiler_patch_address_key(a.address);
                  const uint32_t ba = recompiler_patch_address_key(b.address);
                  if (aa != ba) return aa < ba;
    ...
    h.tag("widescreen_native_patches");
    h.u32((uint32_t)ws_native_patches.size());
    for (const auto& patch : ws_native_patches) {
        h.u32(recompiler_patch_address_key(patch.address));
        h.u32(patch.expected);
        h.u32(patch.replacement);
    }
    h.tag("widescreen_shell_index");
    h.u32(c.ws_shell_index_site);
    h.u32(c.ws_shell_index_expected);
    h.u32(c.ws_shell_base_cols);
    h.u32(c.ws_shell_wide_cols);
...
GameConfig load_game_config(const fs::path& config_path_in) {
    ...
    int ws_camera_zoom_num = 1;
    int ws_camera_zoom_den = 1;
    ...
    uint32_t ws_nw_hud_packet2_lo = 0;
    uint32_t ws_nw_hud_packet2_hi = 0;
    bool ws_nw_backdrop = false;
    bool ws_nw_2d_edge_fill = false;
    bool ws_clear_reveal = false;
    bool ws_nw_flat_backdrop = false;
    bool ws_nw_phase_backdrop = false;
    int ws_nw_ui_stretch_min_ot = 0;
    ...
    std::vector<RecompilerPatch> ws_native_patches;
    uint32_t ws_shell_index_site = 0;
    uint32_t ws_shell_index_expected = 0;
    uint32_t ws_shell_base_cols = 0;
    uint32_t ws_shell_wide_cols = 0;
    if (cfg.contains("video")) {
        const toml::value& video = toml::find(cfg, "video");
        if (video.contains("offer_vulkan"))
    ...
    if (cfg.contains("widescreen")) {
        ...
        if (ws.contains("native_patch")) {
            const auto& patches = toml::find<toml::array>(ws, "native_patch");
            std::set<std::string> seen_ids;
            std::set<uint32_t> seen_addresses;
            for (const auto& item : patches) {
                RecompilerPatch patch;
                patch.id = toml::find<std::string>(item, "id");
                patch.address = parse_hex(
                    toml::find<std::string>(item, "address"),
                    "widescreen.native_patch.address");
                patch.expected = parse_hex(
                    toml::find<std::string>(item, "expected"),
                    "widescreen.native_patch.expected");
                patch.replacement = parse_hex(
                    toml::find<std::string>(item, "replacement"),
                    "widescreen.native_patch.replacement");
                if (item.contains("note"))
                    patch.note = toml::find<std::string>(item, "note");
                if (patch.id.empty())
                    throw std::runtime_error(
                        "widescreen.native_patch.id must not be empty");
                if ((patch.address & 3u) != 0u)
                    throw std::runtime_error(fmt::format(
                        "{}: widescreen native patch '{}' address 0x{:08X} "
                        "is not instruction-aligned",
                        config_path.string(), patch.id, patch.address));
                if (!seen_ids.insert(patch.id).second)
                    throw std::runtime_error(fmt::format(
                        "{}: duplicate [[widescreen.native_patch]] id '{}'",
                        config_path.string(), patch.id));
                const uint32_t key =
                    recompiler_patch_address_key(patch.address);
                if (!seen_addresses.insert(key).second)
                    throw std::runtime_error(fmt::format(
                        "{}: duplicate [[widescreen.native_patch]] physical "
                        "address 0x{:08X}", config_path.string(), key));
                ws_native_patches.push_back(std::move(patch));
            }
        }
        if (ws.contains("shell")) {
            const toml::value& shell = toml::find(ws, "shell");
            const bool has_site = shell.contains("index_site");
            const bool has_expected = shell.contains("index_expected");
            const bool has_base = shell.contains("base_cols");
            const bool has_wide = shell.contains("wide_cols");
            const int present = (int)has_site + (int)has_expected +
                                (int)has_base + (int)has_wide;
            if (present != 0 && present != 4)
                throw std::runtime_error(fmt::format(
                    "{}: [widescreen.shell] index_site, index_expected, "
                    "base_cols, and wide_cols must be set together",
                    config_path.string()));
            if (present == 4) {
                ws_shell_index_site = parse_hex(
                    toml::find<std::string>(shell, "index_site"),
                    "widescreen.shell.index_site");
                ws_shell_index_expected = parse_hex(
                    toml::find<std::string>(shell, "index_expected"),
                    "widescreen.shell.index_expected");
                ws_shell_base_cols = (uint32_t)toml::find<int>(
                    shell, "base_cols");
                ws_shell_wide_cols = (uint32_t)toml::find<int>(
                    shell, "wide_cols");
                if ((ws_shell_index_site & 3u) != 0u ||
                    ws_shell_base_cols == 0u ||
                    ws_shell_wide_cols <= ws_shell_base_cols ||
                    ((ws_shell_wide_cols - ws_shell_base_cols) & 1u) != 0u)
                    throw std::runtime_error(fmt::format(
                        "{}: invalid [widescreen.shell] dimensions/site",
                        config_path.string()));
            }
        }
        ...
        const bool has_camera_zoom_num = ws.contains("camera_zoom_num");
        const bool has_camera_zoom_den = ws.contains("camera_zoom_den");
        if (has_camera_zoom_num != has_camera_zoom_den)
            throw std::runtime_error(fmt::format(
                "{}: [widescreen] camera_zoom_num and camera_zoom_den must be set together",
                config_path.string()));
        if (has_camera_zoom_num) {
            ws_camera_zoom_num = toml::find<int>(ws, "camera_zoom_num");
            ws_camera_zoom_den = toml::find<int>(ws, "camera_zoom_den");
            if (ws_camera_zoom_num <= 0 || ws_camera_zoom_den <= 0 ||
                ws_camera_zoom_num > ws_camera_zoom_den)
                throw std::runtime_error(fmt::format(
                    "{}: [widescreen] camera zoom must be in the range (0, 1]",
                    config_path.string()));
        }
        ...
        const bool has_nw_hud_packet2_lo = ws.contains("nw_hud_packet2_lo");
        const bool has_nw_hud_packet2_hi = ws.contains("nw_hud_packet2_hi");
        if (has_nw_hud_packet2_lo != has_nw_hud_packet2_hi)
            throw std::runtime_error(fmt::format(
                "{}: [widescreen] nw_hud_packet2_lo and "
                "nw_hud_packet2_hi must be set together",
                config_path.string()));
        if (has_nw_hud_packet2_lo) {
            ws_nw_hud_packet2_lo = parse_hex(
                toml::find<std::string>(ws, "nw_hud_packet2_lo"),
                "widescreen.nw_hud_packet2_lo");
            ws_nw_hud_packet2_hi = parse_hex(
                toml::find<std::string>(ws, "nw_hud_packet2_hi"),
                "widescreen.nw_hud_packet2_hi");
            if (ws_nw_hud_packet2_lo >= ws_nw_hud_packet2_hi)
                throw std::runtime_error(fmt::format(
                    "{}: [widescreen] nw_hud_packet2 range is empty or reversed",
                    config_path.string()));
        }
        if (ws.contains("nw_backdrop"))
            ws_nw_backdrop = toml::find<bool>(ws, "nw_backdrop");
        if (ws.contains("nw_2d_edge_fill"))
            ws_nw_2d_edge_fill = toml::find<bool>(ws, "nw_2d_edge_fill");
        ...
        if (ws.contains("nw_ui_stretch_min_ot")) {
            ws_nw_ui_stretch_min_ot = toml::find<int>(ws, "nw_ui_stretch_min_ot");
            if (ws_nw_ui_stretch_min_ot < 0 || ws_nw_ui_stretch_min_ot > 65535)
                throw std::runtime_error(fmt::format(
                    "{}: [widescreen] nw_ui_stretch_min_ot must be 0..65535",
                    config_path.string()));
        }
    ...
    std::vector<uint32_t> ws_cull_bias_sites, ws_cull_right_sub_sites,
        ws_cull_range_sites, ws_cull_a1_sites;
    std::vector<uint32_t> ws_cull_screen_x_sites;
    uint32_t ws_cull_screen_x_expected = 0;
    ...
    int ws_background_guard_pixels = 0;
    ...
    if (cfg.contains("widescreen")) {
        ...
        if (ws.contains("cull")) {
            ...
            load_sites("right_sub_sites", ws_cull_right_sub_sites);
            ...
            if (cull.contains("screen_x_expected")) {
                const int64_t value =
                    toml::find<int64_t>(cull, "screen_x_expected");
                if (value <= 0 || value > UINT32_MAX) {
                    throw std::runtime_error(fmt::format(
                        "{}: widescreen.cull.screen_x_expected must be a "
                        "positive 32-bit integer",
                        config_path.string()));
                }
                ws_cull_screen_x_expected = static_cast<uint32_t>(value);
                if ((ws_cull_screen_x_expected >> 26) != 0x0Bu) {
                    throw std::runtime_error(fmt::format(
                        "{}: widescreen.cull.screen_x_expected must be SLTIU",
                        config_path.string()));
                }
            }
                ...
                for (const auto& item :
                    ...
                    if (!seen.insert(site.address & 0x1FFFFFFFu).second)
                        throw std::runtime_error(fmt::format(
                            ...
                            config_path.string(), site.address));
                    if (item.contains("full_angle"))
                        site.full_angle = toml::find<bool>(item, "full_angle");
            ...
            if (cull.contains("background_guard_pixels")) {
                ws_background_guard_pixels =
                    toml::find<int>(cull, "background_guard_pixels");
                if (ws_background_guard_pixels < 0 ||
                    ws_background_guard_pixels > 32768)
                    throw std::runtime_error(fmt::format(
                        "{}: [widescreen.cull] background_guard_pixels must "
                        "be in [0, 32768]", config_path.string()));
            }
    ...
    std::vector<uint32_t> ws_backdrop_packet_tag_sites;
    std::vector<uint32_t> ws_backdrop_unsquash_funcs;
    std::vector<uint32_t> ws_dome_call_sites;
    int ws_dome_overscan_num = 1;
    int ws_dome_overscan_den = 1;
    int ws_dome_min_sz = 0;
    if (cfg.contains("widescreen")) {
        const toml::value& ws = toml::find(cfg, "widescreen");
        if (ws.contains("backdrop")) {
            ...
            if (bd.contains("packet_tag_sites"))
                for (const auto& a : toml::find<std::vector<std::string>>(bd, "packet_tag_sites"))
                    ws_backdrop_packet_tag_sites.push_back(
                        parse_hex(a, "widescreen.backdrop.packet_tag_sites"));
    ...
    if (cfg.contains("widescreen")) {
        ...
        if (ws.contains("dome")) {
            ...
            if (dome.contains("overscan_num"))
                ws_dome_overscan_num = toml::find<int>(dome, "overscan_num");
            if (dome.contains("overscan_den"))
                ws_dome_overscan_den = toml::find<int>(dome, "overscan_den");
            if (dome.contains("min_sz"))
                ws_dome_min_sz = toml::find<int>(dome, "min_sz");
            if (ws_dome_overscan_num <= 0 || ws_dome_overscan_den <= 0 ||
                ws_dome_min_sz < 0)
                throw std::runtime_error(
                    "widescreen.dome overscan ratio must be positive and min_sz non-negative");
        }
    }
    ...
    return GameConfig{
        ...
        /*ws_background_guard_pixels*/ ws_background_guard_pixels,
        ...
        /*ws_backdrop_packet_tag_sites*/ ws_backdrop_packet_tag_sites,
        /*ws_backdrop_unsquash_funcs*/ ws_backdrop_unsquash_funcs,
        /*ws_dome_call_sites*/    ws_dome_call_sites,
        /*ws_dome_overscan_num*/  ws_dome_overscan_num,
        /*ws_dome_overscan_den*/  ws_dome_overscan_den,
        /*ws_dome_min_sz*/        ws_dome_min_sz,
        /*ws_auto_screen_x_cull*/ ws_auto_screen_x_cull,
        /*ws_auto_backdrop_preload*/ ws_auto_backdrop_preload,
        /*ws_camera_zoom_num*/   ws_camera_zoom_num,
        /*ws_camera_zoom_den*/   ws_camera_zoom_den,
        ...
        /*ws_nw_hud_packet2_lo*/    ws_nw_hud_packet2_lo,
        /*ws_nw_hud_packet2_hi*/    ws_nw_hud_packet2_hi,
        /*ws_nw_backdrop*/        ws_nw_backdrop,
        /*ws_nw_2d_edge_fill*/    ws_nw_2d_edge_fill,
        /*ws_clear_reveal*/       ws_clear_reveal,
        /*ws_nw_flat_backdrop*/   ws_nw_flat_backdrop,
        /*ws_nw_phase_backdrop*/  ws_nw_phase_backdrop,
        /*ws_nw_ui_stretch_min_ot*/ ws_nw_ui_stretch_min_ot,
        ...
        /*ws_native_patches*/     ws_native_patches,
        /*ws_shell_index_site*/   ws_shell_index_site,
        /*ws_shell_index_expected*/ ws_shell_index_expected,
        /*ws_shell_base_cols*/    ws_shell_base_cols,
        /*ws_shell_wide_cols*/    ws_shell_wide_cols,
    };
}
