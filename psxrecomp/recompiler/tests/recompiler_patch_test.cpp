...
xclip_load_sites = ["0x80012340"]
    ...
    const auto screen_x_path = write_config(root, "screen-x-cull", R"toml(
[widescreen.cull]
screen_x_sites = ["0x80012340", "0x80012344"]
screen_x_expected = 0x2C420170
)toml");
    const auto screen_x_guard_config =
        PSXRecompV4::load_game_config(screen_x_path);
    check(screen_x_guard_config.ws_cull_screen_x_sites ==
              std::vector<uint32_t>{0x80012340u, 0x80012344u} &&
              screen_x_guard_config.ws_cull_screen_x_expected ==
                  0x2C420170u,
          "parser preserves shared exact screen-X instruction guard");
    auto changed_screen_x_config = screen_x_guard_config;
    changed_screen_x_config.ws_cull_screen_x_expected = 0x2C420171u;
    check(PSXRecompV4::overlay_codegen_config_hash(
              screen_x_guard_config) !=
              PSXRecompV4::overlay_codegen_config_hash(
                  changed_screen_x_config),
          "screen-X expected word changes overlay cache identity");

    const auto bad_screen_x =
        write_config(root, "screen-x-cull-bad-expected", R"toml(
[widescreen.cull]
screen_x_sites = ["0x80012340"]
screen_x_expected = 0x28420170
)toml");
    check_throws(
        [&] { (void)PSXRecompV4::load_game_config(bad_screen_x); },
        "screen_x_expected must be SLTIU",
        "parser rejects a non-SLTIU screen-X expected word");
...
void codegen_tests() {
    ...
    PSXRecomp::CodeGenConfig screen_x_config;
    screen_x_config.ws_cull_screen_x_sites.insert(0x80010000u);
    screen_x_config.ws_cull_screen_x_expected = 0x2C420170u;
    const std::string screen_x = generate_first_instruction(
        0x2C420170u, {}, false, screen_x_config); // sltiu v0,v0,368
    check(screen_x.find("psx_ws_cull_sltiu(cpu->gpr[2], 368)") !=
              std::string::npos,
          "codegen applies screen-X widening on an exact full-word match");
    check_throws(
        [&] {
            (void)generate_first_instruction(
                0x2C820170u, {}, false,
                screen_x_config); // still SLTIU, but rs differs
        },
        "expected instruction 0x2C420170, found 0x2C820170",
        "codegen rejects same-opcode screen-X word drift");
    const std::string screen_x_overlay_mismatch =
        generate_first_instruction(0x2C820170u, {}, true,
                                   screen_x_config);
    check(screen_x_overlay_mismatch.find("ws explicit screen-x cull") ==
              std::string::npos,
          "overlay full-word mismatch leaves screen-X site unchanged");

    PSXRecomp::CodeGenConfig legacy_screen_x_config;
    legacy_screen_x_config.ws_cull_screen_x_sites.insert(0x80010000u);
    const std::string legacy_screen_x = generate_first_instruction(
        0x2C820170u, {}, false, legacy_screen_x_config);
    check(legacy_screen_x.find("ws explicit screen-x cull") !=
              std::string::npos,
          "zero expected word retains opcode-only screen-X compatibility");
    ...
    auto full_angle_config = angle_config;
    full_angle_config.ws_cull_angle_sites[0].full_angle = true;
    const std::string full_angle = generate_first_instruction(
        0x24020155u, {}, false, full_angle_config);
    check(full_angle.find("psx_ws_full_angle_widen(341u)") != std::string::npos,
          "full apertures use the live-width helper, not the half-angle helper");
