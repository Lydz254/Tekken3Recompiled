...
def check_delay_slot_prefix_capture_normalization():
    # The short capture ends exactly on a JAL.  Two overlapping captures agree
    # on both the whole source identity and the real delay-slot word.
    prefix = bytearray(8)
    put(prefix, 0x00, 0x27BDFFF0)
    put(prefix, 0x04, jal(LOAD + 0x40))
    delay_word = 0x24020001
    extension_a = prefix + struct.pack('<II', delay_word, 0x03E00008)
    # Exercise an overlapping provider with a different load base as well as
    # the ordinary same-base coalesced capture.
    extension_b = (struct.pack('<I', 0x00000000) + prefix +
                   struct.pack('<II', delay_word, 0x03E00008))

    def capture(data, dispatch=(), executed=()):
        return {
            'schema': 'psxrecomp overlay capture v2',
            'load_addr': f'0x{LOAD:08X}',
            'size': len(data),
            'bytes_b64': base64.b64encode(data).decode(),
            'dispatch_entry_pcs': [f'0x{pc:08X}' for pc in dispatch],
            'executed_pcs': [f'0x{pc:08X}' for pc in executed],
            'function_entry_pcs': [],
            'seeds': [f'0x{pc:08X}' for pc in dispatch],
            'static_dispatch_entry_pcs': [],
        }

    short = capture(bytes(prefix),
                    dispatch=(LOAD,),
                    executed=(LOAD, LOAD + 4, LOAD + 0x100))
    long_a = capture(bytes(extension_a), dispatch=(LOAD + 8,))
    long_b = capture(bytes(extension_b), dispatch=(LOAD + 12,))
    long_b['load_addr'] = f'0x{LOAD - 4:08X}'
    normalized, events = MOD.complete_delay_slot_boundary_captures(
        [short, long_a, long_b])
    assert len(events) == 1
    assert events[0]['original_size'] == len(prefix)
    assert events[0]['provider_count'] == 2
    assert events[0]['delay_word'] == delay_word
    assert len(normalized) == 3
    repaired = normalized[0]
    assert repaired['size'] == len(prefix) + 4
    assert base64.b64decode(repaired['bytes_b64']) == \
        bytes(prefix) + struct.pack('<I', delay_word)
    assert repaired['dispatch_entry_pcs'] == [f'0x{LOAD:08X}']
    assert repaired['executed_pcs'][-1] == f'0x{LOAD + 0x100:08X}'
    # Input records are not mutated; callers may reuse the retained vault data.
    assert short['size'] == len(prefix)

    ordinary = capture(struct.pack('<II', 0x27BDFFF0, 0x24020001),
                       dispatch=(LOAD,))
    ordinary_long = capture(
        struct.pack('<III', 0x27BDFFF0, 0x24020001, 0x03E00008))
    unchanged, events = MOD.complete_delay_slot_boundary_captures(
        [ordinary, ordinary_long])
    assert len(unchanged) == 2 and not events

    # Exact-prefix agreement is mandatory.  Conflicting captured delay words
    # are temporal ambiguity, so retain the loud truncated recipe unchanged.
    conflict = capture(bytes(prefix) + struct.pack('<I', 0x24020002))
    ambiguous, events = MOD.complete_delay_slot_boundary_captures(
        [short, long_a, conflict])
    assert not events
    assert ambiguous[0]['size'] == len(prefix)
...
def main():
    ...
    check_delay_slot_prefix_capture_normalization()
