...
struct CodeGenConfig {
    ...
    // Exact packet-finalizer stores for background-only sidecar edge expansion.
    std::set<uint32_t> ws_backdrop_packet_tag_sites;
    ...
    std::set<uint32_t> ws_cull_right_sub_sites;
    std::set<uint32_t> ws_cull_range_sites;
    std::set<uint32_t> ws_cull_a1_sites;
    std::set<uint32_t> ws_cull_screen_x_sites;
    // Optional exact MIPS-word guard shared by ws_cull_screen_x_sites.
    // Zero retains the legacy SLTIU-opcode-only behavior.
    ...
    // Full-word-guarded widescreen instruction alternatives compiled into the
    // AOT function. Vanilla remains the inactive/4:3 path; no text page is
    // modified at runtime. See [[widescreen.native_patch]].
    std::vector<PSXRecompV4::RecompilerPatch> ws_native_patches;

    // Row-major widened-shell source-index store. The emitted helper wraps the
    // added outer row/column onto valid stock descriptors while preserving the
    // independently widened world coordinates.
